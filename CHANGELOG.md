...
- **refactor**: 引入 PRD-to-AI-Architecture 方法论并与 ProofLoop 工作流深度整合
  - `.agents/skills/prd-to-ai-architecture/`: 部署方法论新技能，并在 SKILL.md 的 Required Outputs 中增加 tech-spec.md 描述，同时新增了对应的模版文件。
  - `.agents/skills/README.md`: 注册新技能说明，更正 v3.3 职责模型以正确表述 Brain 加载技能（prd-to-tech-design-prep 和 prd-to-ai-architecture）、@general 仅负责文件持久化的界限。
  ...
  - `tech-spec.md` & `tech-spec.md.example`: 新建项目根目录全局技术规范与架构入口索引文件并填入 ProofLoop 真实技术栈约束，同步更新 example 模版文件以保持其示范引导功能。

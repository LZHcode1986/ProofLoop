...
|---|---|---|---|---|---|
| 1 | Brain 直接操作 Git（commit/reset/checkout） | 已闭环 | Brain 只读 Git（status/log/diff/show）；一切 Git 写操作派 Committer；Committer 是唯一 Git boundary 建立者 | `.pi/brain-workflow.md`（Brain 不执行 Committer 职责）；`.agents/skills/proofloop-execute/references/committer-template.md`（唯一提交者契约） | Stage Review + 本对照表审计（FR-024 矩阵存在性/覆盖测试） |
...
| 3 | `git checkout --` 误操作丢失已实现代码 | 已闭环 | Git 写操作一律派 Committer（同 #1）；还原文件前先 `git status`/`git diff` 确认内容归属；临时编辑用手动反向编辑而非 checkout 整个文件 | `.pi/brain-workflow.md`（Git 只读纪律）；`docs/流程执行问题.md` #3 正确处理与核心教训 #1 | Stage Review + 本对照表审计 |

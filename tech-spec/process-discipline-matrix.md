...
| 11 | 用 general 承载 Worker 任务 | Worker 必须由 Runtime Primary Next Action 和 active Worker Skill/template 路由；`subagent` transport 在 Session 创建时固定，不能由临时 Agent 或叙事替换 | `.agents/skills/proofloop-worker/SKILL.md`；`.agents/skills/proofloop-execute/SKILL.md`；`.pi/brain-workflow.md` | dispatch packet、route 和 recovery 审计 |

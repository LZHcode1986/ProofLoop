# Contract & State Matrix — ProofLoop v2 通用 CLI（OpenCode Plugin 已退役）

> 权威来源：`PRD.md` FR-001..024；kernel 契约（`packages/kernel/src/contracts.ts`、`validators.ts`、`types.ts`）；Runtime 服务（`packages/runtime/src/*`）。当前字段、状态和 Receipt 闭集以这些来源及本矩阵为准；未随模板发布的历史输入不作为引用。状态标注：confirmed / assumed / open。`contracts.ts` 的 16 值 `ReceiptType` 是 canonical source。
...
ok: true | false
command:
  domain: authority | plan | context | stage | review | project | doctor | gate | recovery | cutover | boundary
...
| `stage` | `status`, `next`, `admit-worker`, `admit-cv`, `admit-slice-commit`, `admit-integration`, `run-gate`, `close` | Stage state/next；受理 Worker/CV/Slice Commit/Integration 结构化结果；`run-gate` 当前仅作闭集注册/校验，无 handler，返回 typed `RUNTIME.NOT_IMPLEMENTED`；验证 Stage Close 前置条件 | NextAction/Context/Receipt/Review refs |
...
| `gate` | `run`, `status` | 读取当前 Stage/Slice facts 并校验或报告集成事实 Gate；只写 Runtime-owned Gate Receipt；不执行 Manifest `runtime_proof`，不绑定 `runtime_proof_digest` | Stage Gate refs/verdict/findings |
| `recovery` | `check`, `preflight`, `restart`, `doctor` | 检查中断交易、Receipt/Context/Manifest 链和可恢复性；返回 typed blocker 或恢复入口 | Recovery findings/refs |
| `cutover` | `status`, `execute` | Historical/已关闭迁移清理的只读扫描与显式、不可逆清单执行；不恢复 Plugin，不作为日常执行入口 | Legacy scan/delete findings |
| `boundary` | `close` | 在 Runtime 校验 boundary type、root、scope、HEAD 与 dirty 状态后执行受保护 Git boundary；`artifact-archive` 的纯 rename 例外仍由其合同约束 | Commit/Archive facts 与 boundary findings |
| `doctor` | `run`, `status` | Runtime/CLI 版本、Git、project root、schema、commands/services、filesystem capability；Host API 项仅在 adapter 自身诊断 | structured diagnostics |

Unknown domain/operation 必须在任何 filesystem write 前返回 `RUNTIME.SCHEMA_MISMATCH`。

- §0.3 是 public `proofloop` operation 的唯一权威清单；历史 Host/Plugin operation（如 `admit_spv_result`）不是 public CLI alias，不得混用。source `runPlan`、built CLI 和 parity fixture 必须断言同一 closed set。
...
| Initial CV | pre-refutation Authority/goal/proof、代码/测试/diff/snapshot refs；CV Context 必须绑定 Manifest Slice，Task-level Evidence gate 必须再绑定已验证 task anchor；`evidence_read:false` | Worker Evidence 正文/摘要（观察固定前） |
...
### CV Evidence gate 与当前 route 状态（confirmed / open）

- `context prepare` 通用 request 强制 `role` 与 `stage`；CV role 的 Runtime projection 还强制 Manifest-declared `slice`。`task` 在通用 projection API 中可省略，但可用的 Task-level Evidence gate 必须使用已验证 task anchor，调用 `context prepare --role cv --stage <stage-id> --slice <slice-id> --task <task-id>`；taskless CV Context 不能与 observation 的 task tuple 匹配。
- `context admit-refutation-observation` 是 CV-only，明确要求 `stage`、`slice`、`task` 与非空 `observation`，且要求先存在同一 stage/slice/task、Manifest/Plan/snapshot 绑定的 Runtime-persisted CV Context；它写入 `REFUTATION_OBSERVATION`，其中 `evidence_read: false`。它不是 `CV_RESULT` 字段，也不能由 caller JSON 或模型叙事代替。
- `context show` 通用 request 强制 `role` 与 `stage`；要证明 CV Evidence gate，必须读回同一 task-bound `context_ref` 并提供 Runtime-persisted `observation_ref`（例如 `context show --role cv --stage <stage-id> --slice <slice-id> --task <task-id> --ref <pre-refutation-context-ref> --observation-ref <observation-ref>`）。Runtime 会重验 stage/slice/task/evidence_path/snapshot 与 digest；没有或不匹配 observation 时 `gate.satisfied` 不为 true，只有 gate 满足后才可读取 Evidence。
- `CV_RESULT` 的 closed payload 必须包含 `worker_receipt_digest`；`validateVNextCVResultEnvelope` / `admitVNextCVResult` 将其与当前 Slice 的 Worker（`TASK_COMPLETE`）Receipt chain tip 精确比较，不能只引用 chain member。结果同时须绑定最终 Worker Context；task/observation handoff 是独立 Context gate fact，不应向 closed `CV_RESULT` 增加未知字段。
- **当前 blocker（open）**：`stage next` 投影的 `RUN_CV` 当前只返回 stage/slice/Manifest/Plan/Proof Index/snapshot 等字段，不返回 `task_id`、`worker_receipt_digest`（也不返回 CV Context ref）；`stage admit-cv` 只接收/验证 closed `CV_RESULT` 并调用 vNext CV admission，不接收或消费 `observation_ref`/`context show` gate。因此当前 `RUN_CV → Context gate → CV_RESULT → stage admit-cv` route 未闭合；不能把现有 direct CV credential/admission 写成 Evidence-gated executable `PASS` chain。缺少可验证 handoff 时按既有 `RUNTIME_BLOCKER` 路由返回非 admission `BLOCKED`，不得声称 `PASS`。
...
## 1. Historical Host Tool Contracts

> OpenCode Plugin tools were retired on 2026-08-14. The current semantic/field/status authority is the public CLI contract in §0, the file/data contracts in §3–§4, and the Runtime/kernel implementation. No Plugin tool operation is a current command surface.

## 2. Historical Host Hook Contracts
> OpenCode lifecycle hooks were a historical adapter mechanism, not a current state or field authority. Current protected-path, dirty-state, command-policy and recovery behavior is expressed by Runtime/CLI boundaries and the applicable Agent/Skill instructions; this section carries no active hook contract.

## 3. File / Artifact Contracts

| Path | Producer | Consumer | Format | Required fields | Lifecycle | Validation |
|---|---|---|---|---|---|---|
| `.proofloop/runtime.lock` | 项目初始化/Runtime bootstrap | Runtime/CLI `doctor` 与启动检查 | JSON | runtime_version, domain_schema_version, risk_policy_version, capability_policy_version, host_adapter, extension_package, extension_version | 版本不兼容 → fail closed | kernel `validateRuntimeLock` |
| `.proofloop/manifests/<stage>.json` | Runtime v1/vNext compiler | runtime 全链 | JSON（Manifest） | v1 legacy schema 或 v2 `plan`、`reference_index`、Proof Index、dependencies、skills、Evidence paths、以及可选的 Kernel-兼容 `runtime_proof` | 不可变（digest 绑定）；v2 active compile 当前不生成 `runtime_proof`，不将其视为 Gate 执行契约 | kernel v1/vNext validator；Runtime/CLI current consumer 不执行该字段 |
...
| `.proofloop/logs/**` | Runtime CLI | 诊断 | 文本 | — | 追加 | — |
| `.proofloop/context/<digest>.json` | Runtime Context Resolver（经 public CLI） | 当前 role Agent / admission | JSON（derived Context） | role/action、input refs/digests、snapshot、scope、self digest | write-once、可重建、非 Receipt authority | root/digest/identity + source bindings |
...
| Manifest | stage_id, slices；Kernel 仍可兼容可选 `runtime_proof`；v2 另含 plan/reference_index/Proof Index/digests | kernel contracts.ts + vNext contracts | 写：Runtime compiler；读：Runtime（current Gate 不执行 `runtime_proof`） | validate v1/vNext schema；active compile 未生成该字段 | 版本演进需重新编译；不得隐式迁移；未来 executable proof 需单独立项 |
...
| ExecutableRuntimeProofStep（Historical/open） | Kernel 可选兼容类型：id, type(`command\|service_start\|service_stop\|probe`), executable, args, cwd, timeout_ms, expected；按 type 可有 readiness_signal/service_ref；`not_applicable.reason` 为显式例外 | kernel Runtime Proof contract（compatibility only） | 当前 active vNext 无 producer/consumer；Gate 不执行，且不绑定 `runtime_proof_digest` | Kernel validator 仍拒绝 unknown field/type、非 root cwd、非正 timeout 等；caller-supplied digest 不成为当前 CLI 输入 | 未来若重新立项需另行建立 candidate/compiler/dispatch/Gate/process evidence；不以兼容类型或 S09/S10 历史规划声称完成 |
| EvidenceRefreshRequest | stage_id, manifest_ref/digest, expected_previous_manifest_digest, evidence refs, `mode` (`refresh`\|`recover`\|`rollback`\|`replan`) | Runtime Evidence-refresh/Replan epoch seam（S09 executable bootstrap 为历史/open） | 写：Runtime `plan refresh-evidence`；读：Planning/SPV/epoch admission | `refresh|recover|rollback` 仅 pre-admission pristine；`replan` 绑定 current/parent epoch、Disposition 并执行安全 rotation；全量预检 + transaction journal + compare-and-swap | 任何 non-pristine/stale/symlink、父链不一致或未恢复 transaction 直接阻断；不得据此推断 Runtime Proof 已可执行 |
...
| VNextCvResultEnvelope | nested `CV_RESULT`：`schema_version` (`2`\|`3`), `type`, stage/slice/Manifest/Plan/Proof Index/Context/snapshot bindings；`worker_receipt_digest` 必填且必须精确等于当前 Slice Worker（`TASK_COMPLETE`）Receipt chain tip；verification/refs/findings；v3 另含 `stage_contract_digest`, `slice_contract_digest`, `execution_binding_digest`；task/observation 是独立 Context gate facts | Code Verifier → `stage admit-cv` → shared validator → vNext CV consumer（当前 `stage admit-cv` 不消费 observation） | 写：Runtime CV admission；读：`stage next`/Gate/Review/currentness | Manifest mode gate、Worker tip、Context、Proof Index、epoch/path/snapshot 与三层 binding 全部一致；v2/v3 混链 fail-closed；但 `RUN_CV` 当前不返回 `task_id`/Worker tip，CV Evidence-gated route 未闭合，不得宣称 executable `PASS` chain | 外层 CLI schema 2 不得遮蔽 nested v3；v3 不得被 v2 consumer 消费；未完成 gate handoff 时不得进入 admission
...
## 5. State Machines（kernel 权威，Runtime/CLI 只读投影）
...
| CVStatus | NOT_STARTED, READY_FOR_CV, IN_PROGRESS, PASS, REPAIR, PENDING_RECHECK | kernel `transitionCv`（6 值闭集） | CV receipt / vNext execution fact 派生 | public `proofloop stage status|next` + Context/Evidence projection |
...
## 5.1 Historical S08 vNext contracts（现行语义由 Runtime/CLI consumer 承接）

以下合同保留稳定 S08 entity id 以兼容历史引用；当前字段/状态权威仍来自 kernel/Runtime 与本矩阵。candidate `tasks.md` 只能通过稳定 entity ref 引用它们。

### S08-A routing seam
<!-- proofloop:entity id="PLUGINV2-S08-A-SEAM" kind="seam" -->
- Public seam：public CLI `proofloop plan validate|status`、`proofloop stage status|next`、`proofloop review status`，以及 Runtime 对显式 `version: 1` / `version: 2` Manifest 的 route selection。
- Boundary：v2 只能进入 vNext consumer；v1 保持 legacy boundary；未知 schema、跨版本输入和不可信路径返回 structured Finding，不调用另一版本的 consumer。
- Owner：Runtime 派生 route；CLI 只做 canonical-root/path assertion 和结果投影。
...
- Public seam：public `proofloop stage next` 的 Runtime vNext dispatch projection。
...
- Public seam：同一 fixture 的 Runtime source/dist 与 Runtime CLI 及真实 filesystem/Git 边界；验证结果比较 action、Finding、Receipt/digest 和 fail-closed 行为，而不是只比较内部函数返回值。
...
- Public seam：public CLI `proofloop stage admit-worker|admit-cv|admit-slice-commit|admit-integration`、`proofloop review prepare-stage|finalize-stage` 与 Runtime vNext state/receipt consumers；不得把 v2 envelope 交给 legacy reconcile/reducer。
- State boundary：Stage Plan admission 之后，Runtime 从 persisted facts 依次投影 `DISPATCH_WORKER → TASK_COMPLETE → RUN_CV → CV_PASS/CV_REPAIR → SLICE_COMMIT → INTEGRATION → STAGE_GATE → STAGE_REVIEW`。每个动作只能由唯一 owner 触发，不能由 checkbox、progress 或 Agent narrative 推进。
...
## 5.2 S10 harness-neutral CLI/self-host contracts（当前 public CLI）
...
- Role transfer：Planning/SPV/Worker/CV/Stage Reviewer/Project Reviewer 使用 §0.4 的不同 Context。`context prepare/show` 的 output 必须受 ref/digest/snapshot 绑定，不能由 Brain 复制完整文档或手工拼 Packet。Initial CV 的 Evidence 继续受反驳观察 gate 保护。
...
- Self-host status: public `proofloop` command contract is current；S10 完整 self-host、Execution→Review、Gate/E2E 的 built-process evidence 仍为 open。当前 candidate/materializer 不接受 `runtime_proof` 或 caller `proof_digest`，`plan compile` 不生成该字段；Gate 只校验集成事实，不执行或绑定 Runtime Proof。
- Harness independence: process-level tests 用同一 built executable/request fixture 模拟不同 harness caller；测试不 import harness SDK 或注入 Host tool result。不同 harness 仅有 Agent prompt/permission fixture 差异。
- Cutover（Historical / 已关闭）：旧 CLI/Plugin 切换曾要求完整 source/dist/self-host/restart oracle 通过后清理 v1 consumer；当前不恢复 Plugin，旧制品只按显式 fail-closed 兼容策略读取。
- Failure：unknown domain/op、legacy/unknown/cross-version、stale Authority/Plan/Manifest/Context/snapshot、root escape、不完整 envelope、非法 transition 和 broken Receipt chain 均 canonical Finding + no-write；不得 fallback、放宽断言或以非公共入口掩盖 CLI 缺口。
- Known failures（Historical）：既有 Host 适配失败只作为 CLI 根因分析记录；Runtime/CLI 错误修复并增加 public CLI regression，宿主适配差异不创建新 Plugin 工作流。
- Out of scope（Historical/open）：S08-REVIEW-010 Runtime Proof 回填、S09 executable bootstrap 及其 Manifest/Gate/Review 重验；若恢复必须先重新确认 current contract。

## 5.3 Historical/open Runtime Proof compatibility and Evidence refresh

### S09 bootstrap seam（Historical/open；稳定 entity id 保留历史 `S08B0` 名称）
<!-- proofloop:entity id="PLUGINV2-S08B0-BOOTSTRAP-SEAM" kind="seam" -->
- Current vNext boundary：candidate input/materializer 的 closed schema 不接受 `runtime_proof` 或 caller `proof_digest`；当前 `plan compile` 不生成 `runtime_proof`。Kernel 的可选 Runtime Proof 类型/validator 仅保留兼容语义，不构成当前 producer。
- Current Gate boundary：当前实际 Gate 入口为 `proofloop gate run`，只校验集成事实，不执行 Manifest `runtime_proof`，也不绑定 `runtime_proof_digest`；`stage run-gate` 仅在 closed registry 中注册，无 handler，返回 typed `RUNTIME.NOT_IMPLEMENTED`；没有当前 executable Runtime Proof consumer。
- Canonical Stage ID：Runtime request 的 `stage_id` 只接受 `^S\d+$`；candidate parser、strict compiler、Validator、status 与 admission 共用该规则。旧 `S08B0`/`S08B` 只作历史导航，不得进入 Runtime request；这条 ID 规则不表示 S09 bootstrap 已完成。
- Historical/open S10 rule：历史规划曾要求 S10 ProofSpecification 含非 `not_applicable` step；当前 active compile 不生成 `runtime_proof`，Gate 不执行该字段，因此该规则不是当前 admission 前置或完成标准。未来恢复时必须重新定义并验证 producer、executor、digest 和 process evidence。
...
- Bootstrap process（Historical/open）：当前没有可执行的 S09 bootstrap seam 或 active admission；不得把 all-skipped `not_applicable`、Kernel 兼容类型或旧 Manifest 当作完成。若未来重新立项，需先实现并验证 producer/compiler/dispatch/Gate/process evidence，再建立 S10 boundary；现行 `plan refresh-evidence` 仅按下述 Evidence rotation contract 工作。

## 6. Ports and Runtime Configuration

| Service | Port | Protocol | Start command | Env | Notes |
|---|---|---|---|---|---|
| 通用 ProofLoop CLI | — | 本地子进程 | `proofloop <domain> <operation>`（repo bootstrap 可用 built entry） | — | 当前主机械表面；harness-neutral；structured JSON；S10 完整 self-host/Gate/E2E 仍 open |
| E2E/Gate 证明步骤（Historical/open） | — | 子进程 | 未来重新立项时才定义 CommandSpec（executable, args, cwd, timeout_ms, expected） | 继承 | 当前 Gate 不消费 Manifest `runtime_proof`；不得把 process-runner/旧 all-skipped 记录当作当前证明 |
...
| 权限拒绝（角色不符） | HOST.CAPABILITY.MISSING | CLI 不允许该角色调用该命令 | 调用者 role + command + 期望角色 | 换正确角色；Runtime/CLI 二次校验 |
| 取消（abort） | AbortError（协作式） | 工具停止执行并返回取消 | 取消信号来源 | 重试；不得吞掉取消 |
| 版本不兼容 | RUNTIME.LOCK_MISMATCH（fail closed） | 提示更新 `runtime.lock` / runtime 版本 | 期望 vs 实际版本 | 更新 lock 或 Runtime |
...
- [x] 每个 public CLI operation 有输入/输出/owner/验证（§0）
- [x] 每个持久化文件有 owner 与 schema（§3）
- [x] Historical Host tool/hook sections 明确标记为非当前入口（§1–§2）
- [x] 每个用户可见错误有日志与结构化输出（§7）
- [x] 状态机有合法转移与持久化（§5）
- [x] Host compatibility 不作为当前 CLI admission 前置；历史 adapter 仅保留兼容审计说明
...
|---|---|---|
| `stage_contract_digest` | `binding_schema_version:1`、`stage_id`、`stage_node{id,kind,goal,refs[],dependencies[],required_skills[]}`（Plan stage 节点不可变投影）、`stage_reference_bindings[{kind,ref_id,file_digest,section_digest}]`（仅 stage 直接引用） | `packages/kernel/src/vnext/bindings.ts`（新增） |
...
| 只改（Historical/未来）runtime proof | 不作为 current vNext input | 不变 | 不适用（current 无该 binding） | 不适用 | 当前 Gate 不执行该字段；若未来重新引入，需另行定义 Slice invalidation 与 Gate rerun |
...
- Stage Gate PASS 必须绑定当前 epoch、current Manifest/Plan、所有当前有效的 Slice binding proof（非 `Manifest.runtime_proof`）和 integrated snapshot；Stage Review ACCEPTED 必须再绑定当前 Gate Receipt；Stage Close 只消费当前 epoch 的 Gate/Review，不得用旧 Receipt 补齐。
...
### S12 binding seam（stable entity id `PLUGINV2-S12-BINDING-SEAM`）
...
### S13 binding-mode seam（stable entity id `PLUGINV2-S13-BINDING-MODE-SEAM`）
...
### S12 test seam（stable entity id `PLUGINV2-S12-TEST-SEAM`）
<!-- proofloop:entity id="PLUGINV2-S12-TEST-SEAM" kind="seam" -->
S12-A 测试基线 seam：fresh clone（无本地 `.proofloop` 遗留）后 Node 25 内置 `node:test` 测试通过；测试以临时 HOME/配置目录隔离运行，不写项目真实配置；`.proofloop/**` 保持 gitignore；fixture 全部自包含（tracked 生成器或静态 fixture）。

### S12 process-fix seam（stable entity id `PLUGINV2-S12-PROCESS-SEAM`）
<!-- proofloop:entity id="PLUGINV2-S12-PROCESS-SEAM" kind="seam" -->
S12-F 流程纪律 seam：materialize 禁令与 replan 投影保留规则由当前 Contract/Runtime 校验承载；Evidence 标题规则由 Worker 派发 Template 与 admission 错误承载；问题→防复发机制→固化位置对照表在 `tech-spec/process-discipline-matrix.md`，每项均有可检查的验证落点。

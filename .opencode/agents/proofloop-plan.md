...
3. 若 material Stage recomposition 或 boundary 不清，按需使用 OpenCode `skill` capability `codebase-design`，执行 Project Stage boundary test；该 capability 只提供 composition evidence，不拥有 Plan、Map 或 route。
...
OpenCode 使用原生 `task` 创建 child、以 child `sessionID` continuation，并读取 returned structured Result/failure；session/transport identity 不是业务事实。
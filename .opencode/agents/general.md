...
目标改动和验证事实必须可由 Brain 重读，且 Result 被正确 consumer 接纳；`idle`、`done`、transport sent、模型摘要或单独测试通过都不算完成。OpenCode 使用 `task` child returned result；不使用第二通道、不自动 retry、不换 role。
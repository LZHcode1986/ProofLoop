...
describe('closeGitBoundary strict dirty gate', () => {
  ...
  test('workflow-contract-update accepts active .pi/extensions and agent config paths, while rejecting arbitrary paths', () => {
    const f = makeFixture();
    try {
      f.write('.pi/extensions/proofloop-mode.ts', 'extension');
      f.write('.agents/agent_config.json', '{"agents":{}}');
      commitAll(f, 'base');
      // Dirty both approved paths so the boundary has changes to commit.
      f.write('.pi/extensions/proofloop-mode.ts', 'extension v2');
      f.write('.agents/agent_config.json', '{"agents":{"worker":{}}}');
      const headBefore = f.head();
      ...
      // A sibling of the exact agent config path is also rejected.
      expectBoundaryCode(
        () =>
          closeGitBoundary(f.dir, {
            boundary_type: 'workflow-contract-update',
            paths: ['.agents/agent_config.json.bak'],
            expected_head: headBefore,
            description: 'agent config sibling',
          }),
        'BOUNDARY.SCOPE_VIOLATION',
      );
      // Approved host and configured-start paths are committed together.
      const declared = ['.agents/agent_config.json', '.pi/extensions/proofloop-mode.ts'];
      const result = closeGitBoundary(f.dir, {
        boundary_type: 'workflow-contract-update',
        paths: declared,
        expected_head: headBefore,
        description: 'configured start scope',
      });

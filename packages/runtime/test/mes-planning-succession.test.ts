...
describe('planning acceptance succession (S06 post-recovery Authority update)', () => {
  ...
  test('req13: the real project MES snapshot is untouched by every isolated fixture in this file', () => {
    // The pre-run observation was captured at module load — before any fixture
    // in this file ran. Re-observe the canonical project/trust root now and
    // require in-run invariance of the OBSERVED bytes and fact identity.
    ...
    // The canonical root is the primary worktree owning the common `.git`
    // directory — never this (possibly linked) worktree and never a nested MES.
    assert.equal(
      fs.statSync(path.join(before.root, '.git')).isDirectory(),
      true,
      'the canonical root must be the primary worktree owning .git',
    );
    assert.equal(before.path, path.join(before.root, CANONICAL_MES_REL));

    const after = observeCanonicalMes();
    ...
    assert.deepEqual(
      after.factIds,
      before.factIds,
      'the canonical project MES fact identity must be unchanged by this run',
    );
  });

  // ---------------------------------------------------------------------------
  // Generation-currentness regressions (Brain-accepted IMPLEMENTATION_DEFECT):
  // currentness must be authoritative for binding-critical SUPPORT resolution,
  // not only for the read projections — the write boundary and the
  // duplicate/ambiguity cohort must agree with the single oracle classification.
  // ---------------------------------------------------------------------------

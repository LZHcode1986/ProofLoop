...
function currentCanonicalPvrFact(verdict: MesPlanVerdict = 'PLAN_READY'): MesFactEnvelope {
  return {
    ...
    scope: { stage_id: 'S06' },
    git_basis: { head: 'c'.repeat(40), branch: 'v2-subagent', worktree: '.' },
  };
}
...
function currentCanonicalPaFact(): MesFactEnvelope {
  return {
    ...
    scope: { stage_id: 'S06' },
    git_basis: { head: 'd'.repeat(40), branch: 'v2-subagent', worktree: '.' },
  };
}
...
function durableSourceIncidentBaselineFact(override?: (baseline: MesFactEnvelope) => MesFactEnvelope): MesFactEnvelope {
  const baseline: MesFactEnvelope = {
    ...
    git_basis: { head: '9'.repeat(40), branch: 'v2-subagent', worktree: '.' },
  };
...
describe('MES disaster recovery baseline (E2E-24)', () => {
  ...
  test('fresh clone does not execute recovery from ignored real incident captures', () => {
    assert.throws(() => observeCanonicalProjectArtifact(`${REAL_FORENSIC_DIR_REL}/snapshot-67.json`), CanonicalProjectMesObservationError);
  });

  test('fresh clone does not read or fallback to ignored forensic captures', () => {
    const canonicalRoot = resolveCanonicalProjectRoot();
    assert.equal(fs.statSync(path.join(canonicalRoot, '.git')).isDirectory(), true);
    for (const artifact of [
      `${REAL_FORENSIC_DIR_REL}/snapshot-67.json`,
      `${S06_BINDING_MISMATCH_DIR_REL}/snapshot-130.json`,
      `${S06_FACT_GAP_DIR_REL}/snapshot-122.json`,
    ]) {
      assert.throws(() => observeCanonicalProjectArtifact(artifact), CanonicalProjectMesObservationError);
    }
  });
});
...
describe('current S06 binding-mismatch recovery baseline (S06-R-F-T01)', () => {
  ...
  test('fresh clone does not require the ignored S06 binding-mismatch capture', () => {
    assert.throws(() => observeCanonicalProjectArtifact(`${S06_BINDING_MISMATCH_DIR_REL}/snapshot-130.json`), CanonicalProjectMesObservationError);
  });
});

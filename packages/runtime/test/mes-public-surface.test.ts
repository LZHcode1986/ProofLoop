...
describe('MES public surface + proofloop status (S01-C-T02)', () => {
  ...
  test('fresh clone does not require ignored historical recovery captures', () => {
    const canonicalRoot = resolveCanonicalProjectRoot();
    assert.equal(fs.statSync(path.join(canonicalRoot, '.git')).isDirectory(), true);
    assert.throws(() => observeCanonicalProjectArtifact(`${REAL_FORENSIC_REL}/snapshot-67.json`), CanonicalProjectMesObservationError);
  });

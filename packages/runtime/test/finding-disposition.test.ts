...
describe('finding disposition arbitration closed set (PO-S03-D-03)', () => {
  ...
  test('AUTHORITY_GAP closes to the authority-owner lane and USER_DECISION_REQUIRED stays a local pause (S06-C-T02 / PO-S06-C-02)', () => {
    ...
    // finding binds its legal claim origin through the two planning review
    // skills (Authority basis in authority_refs, role sources in
    ...
    const gapFinding = findingFact({
      ...
      finding_evidence_refs: [
        '.agents/skills/proofloop-plan/SKILL.md',
        '.agents/skills/stage-plan-verifier/SKILL.md',
      ],
    });
    for (const rel of ['.agents/skills/proofloop-plan/SKILL.md', '.agents/skills/stage-plan-verifier/SKILL.md']) {

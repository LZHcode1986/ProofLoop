...
describe('E2E-25 MES operational transaction regression (S06-R-D-T02)', () => {
  ...
  test('real project .proofloop/mes stays byte-unchanged across the suite (never a mutation target)', () => {
    // The pre-run observation was captured at module load — before any fixture
    // in this suite ran. Re-observe the canonical project/trust root now and
    ...
    // directory — never a fixture root and never a worktree-local/nested MES.
    assert.equal(
  ...
  test('fixture-only discipline: no fixture ever resolves to the real .proofloop/mes path', () => {
    ...
    try {
      ...
      assert.ok(!abs.includes('proofloopv2-wsl-herdr'), 'fixture must never touch the real project tree');
    } finally {
      fixture.cleanup();
    }
  });

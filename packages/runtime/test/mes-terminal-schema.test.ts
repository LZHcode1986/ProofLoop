...
const GIT_BASIS = { head: HEAD, branch: 'v2-herdr', worktree: '.' };
...
describe('MES terminal fact schema (S04-A-T01)', () => {
  test('accepts a closed project_ready terminal fact and rejects inherited Stage/Work/Result bindings', () => {
    ...
    for (const badBasis of [
      undefined,
      null,
      { branch: 'v2-herdr', worktree: '.' }, // missing head
      { head: 'short', branch: 'v2-herdr', worktree: '.' }, // head not 40-hex
      { head: 'Z'.repeat(40), branch: 'v2-herdr', worktree: '.' }, // uppercase hex
      { head: HEAD, worktree: '.' }, // missing branch
      { head: HEAD, branch: '' }, // empty branch
      { head: HEAD, branch: 'v2-herdr', worktree: '../outside' }, // traversal worktree
      { head: HEAD, branch: 'v2-herdr', worktree: '/abs' }, // absolute worktree
    ]) {
      assert.throws(
      ...
      () =>
        validateMesFactEnvelope(
          projectReadyEnvelope(undefined, {
            git_basis: { head: HEAD, branch: 'v2-herdr\u0001', worktree: '.' },
          }),
        ),
      SchemaValidationError,
    ...
    assert.throws(
      () =>
        validateMesFactEnvelope(
          projectReadyEnvelope(undefined, {
            git_basis: { head: HEAD, branch: 'v2-herdr', worktree: '.proofloop/worktrees/S04-S04-A\n' },
          }),
        ),
      SchemaValidationError,
  ...
  test('closes the accepted-stage git_basis shape and project_ready tech-spec authority_refs on the binding path (CV S04-A-01)', () => {
    ...
    for (const badBasis of [
      ...
      { head: 'short', branch: 'v2-herdr', worktree: '.' }, // head not 40-hex
      { head: HEAD, worktree: '.' }, // missing branch
      { head: HEAD, branch: 'v2-herdr' }, // missing worktree
    ]) {
    ...
    assert.equal(
      shape({ git_basis: { head: HEAD, branch: 'v2-herdr', worktree: '.' } }),
      undefined,

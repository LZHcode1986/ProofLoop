...
function sliceDef(sliceId, taskIds, dependencies = []) {
    return {
        slice_id: sliceId,
...
function makeEvidence(sliceId, writtenTasks, finalized = false) {
    const out = [`# Slice ${sliceId} Evidence`, ``, `## Task Evidence`, ``];
...
const allChecked = TASKS_A.map((id) => ({ id, checked: true }));
const allUnchecked = TASKS_A.map((id) => ({ id, checked: false }));
const firstUnchecked = TASKS_A.map((id, i) => ({ id, checked: i > 0 }));
/** Write the standard single-slice (S02-A) manifest + all-checked tasks.md + finalized evidence. */
function writeBaseProject(fx, stageId) {
...
function writePlanReceipts(fx, stageId) {
...
function planOnlyFx(stageId = 'S02') {
    const fx = makeFx(stageId);
    writeBaseProject(fx, stageId);
    const head = fx.commitAll();
    fx.writeReceipt('plan', undefined, {
...
function readyForCvFx(stageId = 'S02') {
...
function cvPassedFx(stageId = 'S02') {
...
function committedFx(stageId = 'S02') {
...
function integratedFx(stageId = 'S02') {
...
function recoverFx(stageId = 'S02') {
    const fx = makeFx(stageId);
    fx.writeManifest([sliceDef('S02-A', TASKS_A)]);
...
function diagnoseFx(stageId = 'S02') {
    const { fx, head } = planFx(stageId);
...
function unresolvedFx(stageId = 'S02') {
    const { fx, head } = planFx(stageId);
    const r1 = fx.writeReceipt('cv', 'S02-A', {
        type: 'CV_REPAIR',
        stage_id: stageId,
...
function recheckFx(stageId = 'S02') {
    const { fx, head } = planFx(stageId);
    fx.writeReceipt('cv', 'S02-A', {
        type: 'CV_REPAIR',
        stage_id: stageId,
...
function gateFailFx(stageId = 'S02') {
    const { fx, head } = planFx(stageId);
    fx.writeReceipt('stage-gate', undefined, {
        type: 'GATE_FAIL',
        stage_id: stageId,
...
function kitchenSinkFx(stageId = 'S02') {
    const fx = makeFx(stageId);
    ...
    const cvDir = (0, runtime_1.receiptCategoryDir)(fx.root, 'cv', stageId, 'S02-A');
...
function depJumpFx(stageId = 'S02') {
    const fx = makeFx(stageId);
    fx.writeManifest([
...
function allBlockedFx(stageId = 'S02') {
    const fx = makeFx(stageId);
    fx.writeManifest([
        sliceDef('S02-A', TASKS_A, ['S02-B']),
...
// ============================================================
(0, vitest_1.describe)('row 0: error-level inconsistency → VALIDATE (real fixtures)', () => {
    (0, vitest_1.it)('GATE_FAIL receipt on a valid chain → VALIDATE with the GATE_FAIL blocking finding, chain stays truthful', () => {
        const { fx } = gateFailFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('VALIDATE');
        (0, vitest_1.expect)(out.action).not.toMatch(EXECUTION_PREFIX);
        ...
        (0, vitest_1.expect)(out.findings.some((f) => f.code === 'DOMAIN.INVALID_TRANSITION' && f.severity === 'error')).toBe(true);
    });
    (0, vitest_1.it)('kitchen-sink inconsistency (reusing S02-C constructions) → VALIDATE, all findings, no execution action', () => {
        ...
        (0, vitest_1.expect)(out.findings.some((f) => f.code === 'RUNTIME.SCHEMA_MISMATCH' && f.severity === 'error')).toBe(true);
        // The recoverable warn finding is returned alongside the errors.
        (0, vitest_1.expect)(out.findings.some((f) => f.code === 'RUNTIME.SCHEMA_MISMATCH' && f.severity === 'warn')).toBe(true);
    });
});
// ============================================================
// Row 1 — stage UNINITIALIZED (no STAGE_PLAN) → VALIDATE fallback
// ============================================================
(0, vitest_1.describe)('row 1: stage UNINITIALIZED → VALIDATE fallback (real fixture)', () => {
    (0, vitest_1.it)('no STAGE_PLAN receipt → VALIDATE with the blocking stage-unplanned finding', () => {
        const { fx } = baseFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('VALIDATE');
        (0, vitest_1.expect)(out.action).not.toMatch(EXECUTION_PREFIX);
        (0, vitest_1.expect)(out.findings.some((f) => f.code === 'DOMAIN.INVALID_TRANSITION' && f.severity === 'error')).toBe(true);
        (0, vitest_1.expect)(out.action_detail).toMatch(/not planned|UNINITIALIZED/);
    });
});
// ============================================================
// Row 2 — stage PLANNING → ADMIT_SPV_RESULT (before slice dispatch)
// ============================================================
(0, vitest_1.describe)('row 2: stage PLANNING → ADMIT_SPV_RESULT (real fixture)', () => {
    (0, vitest_1.it)('STAGE_PLAN without SPV_PASS → ADMIT_SPV_RESULT, even with dispatchable slices (beats implement)', () => {
        const { fx } = planOnlyFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('ADMIT_SPV_RESULT');
        (0, vitest_1.expect)(out.responsible_role).toBe('planner');
        (0, vitest_1.expect)(out.action).not.toMatch(/DISPATCH|RUN_/);
    });
});
// ============================================================
// Row 3 — stage COMPLETED → REPARTITION (3a) / COMPILE_ACCEPTANCE (3b)
// ============================================================
(0, vitest_1.describe)('row 3: stage COMPLETED → REPARTITION / COMPILE_ACCEPTANCE (real fixtures)', () => {
    (0, vitest_1.it)('3b: STAGE_REVIEW_PASS without repartition request → COMPILE_ACCEPTANCE', () => {
        const { fx } = fullFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('COMPILE_ACCEPTANCE');
        (0, vitest_1.expect)(out.receipt_chain_valid).toBe(true);
        (0, vitest_1.expect)(out.findings).toEqual([]);
    });
    (0, vitest_1.it)('3a: manifest repartition_requested=true on COMPLETED → REPARTITION (counterexample)', () => {
        const { fx } = fullFx();
        ...
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('REPARTITION');
        (0, vitest_1.expect)(out.responsible_role).toBe('planner');
        (0, vitest_1.expect)(out.action).not.toBe('COMPILE_ACCEPTANCE');
    });
});
// ============================================================
// Row 4 — pending worker result envelope → ADMIT_WORKER_RESULT
// ============================================================
(0, vitest_1.describe)('row 4: pending worker result envelope → ADMIT_WORKER_RESULT (real fixture)', () => {
    (0, vitest_1.it)('counterexample: envelope pending while the next task is unchecked → ADMIT_WORKER_RESULT, never implement', () => {
        const { fx } = envelopeFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('ADMIT_WORKER_RESULT');
        (0, vitest_1.expect)(out.action_detail).toContain('tok-1');
        (0, vitest_1.expect)(out.action).not.toMatch(/DISPATCH_WORKER/);
    });
});
// ============================================================
// Row 5 — pending CV result envelope → ADMIT_CV_RESULT (not RUN_CV)
// ============================================================
(0, vitest_1.describe)('row 5: pending CV result envelope → ADMIT_CV_RESULT (real fixture)', () => {
    (0, vitest_1.it)('counterexample: CV envelope pending for a READY_FOR_CV slice → ADMIT_CV_RESULT, never RUN_CV', () => {
        ...
        const reconciled = (0, runtime_1.reconcileStage)(input);
        (0, vitest_1.expect)((0, runtime_1.deriveNextAction)(reconciled).action).toBe('RUN_CV');
        ...
        const derived = contractOf((0, runtime_1.deriveNextAction)({
            ...reconciled,
            pending_cv_result_envelopes: [{ stageId: fx.stageId, sliceId: 'S02-A' }],
        }));
        expectContract(derived);
        (0, vitest_1.expect)(derived.action).toBe('ADMIT_CV_RESULT');
        (0, vitest_1.expect)(derived.action).not.toBe('RUN_CV');
        (0, vitest_1.expect)(derived.action).not.toMatch(/DISPATCH/);
    });
    (0, vitest_1.it)('pipeline never guesses a CV envelope (HP-003): a results-dir envelope surfaces as ADMIT_WORKER_RESULT', () => {
        const { fx } = readyForCvFx();
        ...
        const dir = path.join(fx.root, '.pi', 'proofloop-runtime', 'results');
        fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(path.join(dir, 'cv-verdict.json'), JSON.stringify({
            schemaVersion: 1,
            actionToken: 'cv-tok',
            stageId: fx.stageId,
            sliceId: 'S02-A',
        ...
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('ADMIT_WORKER_RESULT');
        (0, vitest_1.expect)(out.action).not.toBe('RUN_CV');
        (0, vitest_1.expect)(out.action).not.toBe('ADMIT_CV_RESULT');
    });
});
// ============================================================
// Row 6 — READY_FOR_CV + latest CV fact CV_REPAIR (repair/diagnose/recheck)
// ============================================================
(0, vitest_1.describe)('row 6: CV_REPAIR branch → repair / diagnose / recheck (real fixtures)', () => {
    (0, vitest_1.it)('6c: CV_REPAIR count 1 → DISPATCH_WORKER mode=repair', () => {
        const { fx } = repairFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('DISPATCH_WORKER');
        (0, vitest_1.expect)(out.action_detail).toContain('mode=repair');
        (0, vitest_1.expect)(out.action_detail).toContain('S02-A');
    });
    (0, vitest_1.it)('6d: CV_REPAIR count 2 → DISPATCH_WORKER mode=diagnose', () => {
        const { fx } = diagnoseFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('DISPATCH_WORKER');
        (0, vitest_1.expect)(out.action_detail).toContain('mode=diagnose');
    });
    (0, vitest_1.it)('6e: CV_REPAIR count ≥ 3 → VALIDATE fallback UNRESOLVED_CV_FAILURE', () => {
        const { fx } = unresolvedFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('VALIDATE');
        (0, vitest_1.expect)(out.action).not.toMatch(EXECUTION_PREFIX);
        (0, vitest_1.expect)(out.findings.some((f) => /UNRESOLVED_CV_FAILURE/.test(f.message))).toBe(true);
        (0, vitest_1.expect)(out.findings.some((f) => f.severity === 'error')).toBe(true);
    });
    (0, vitest_1.it)('6b: repair result admitted after the last CV_REPAIR → RUN_CV recheck', () => {
        const { fx } = recheckFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('RUN_CV');
        (0, vitest_1.expect)(out.action_detail).toContain('recheck');
        (0, vitest_1.expect)(out.responsible_role).toBe('code-verifier');
        (0, vitest_1.expect)(out.action).not.toMatch(/DISPATCH/);
    });
});
// ============================================================
// Row 7 — READY_FOR_CV with no CV receipt → RUN_CV (initial)
// ============================================================
(0, vitest_1.describe)('row 7: READY_FOR_CV without CV receipt → RUN_CV initial (real fixture)', () => {
    (0, vitest_1.it)('finalize-slice admitted, no CV receipt → RUN_CV with action_detail=initial', () => {
        const { fx } = readyForCvFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('RUN_CV');
        (0, vitest_1.expect)(out.action_detail).toContain('initial');
        (0, vitest_1.expect)(out.responsible_role).toBe('code-verifier');
        (0, vitest_1.expect)(out.action).not.toMatch(/DISPATCH/);
    });
});
...
// Row 8 — slice CV_PASSED without SLICE_COMMIT → ADMIT_SLICE_COMMIT
// ============================================================
(0, vitest_1.describe)('row 8: slice CV_PASSED without commit → ADMIT_SLICE_COMMIT (real fixture)', () => {
    (0, vitest_1.it)('CV_PASS admitted, no SLICE_COMMIT → ADMIT_SLICE_COMMIT', () => {
        const { fx } = cvPassedFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('ADMIT_SLICE_COMMIT');
        (0, vitest_1.expect)(out.responsible_role).toBe('committer');
        (0, vitest_1.expect)(out.action_detail).toContain('S02-A');
    });
});
// ============================================================
// Row 9 — slice committed without INTEGRATION_PASS → ADMIT_INTEGRATION
// ============================================================
(0, vitest_1.describe)('row 9: slice committed without integration → ADMIT_INTEGRATION (real fixture)', () => {
    (0, vitest_1.it)('bound SLICE_COMMIT, no INTEGRATION_PASS → ADMIT_INTEGRATION', () => {
        const { fx } = committedFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('ADMIT_INTEGRATION');
        (0, vitest_1.expect)(out.action_detail).toContain('S02-A');
        (0, vitest_1.expect)(out.action).not.toMatch(/DISPATCH/);
    });
});
...
// ============================================================
(0, vitest_1.describe)('row 10a: unstarted slice with missing evidence file → INITIALIZE_EVIDENCE (real fixture)', () => {
    (0, vitest_1.it)('PLANNED slice + evidence file missing → INITIALIZE_EVIDENCE', () => {
        const { fx } = initFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('INITIALIZE_EVIDENCE');
        (0, vitest_1.expect)(out.action_detail).toContain('S02-A');
    });
});
(0, vitest_1.describe)('row 10b: checked ≠ evidence_written → DISPATCH_WORKER mode=recover-task (real fixture)', () => {
    (0, vitest_1.it)('checked task with missing evidence section → recover-task + warn finding', () => {
        const { fx } = recoverFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('DISPATCH_WORKER');
        (0, vitest_1.expect)(out.action_detail).toContain('mode=recover-task');
        (0, vitest_1.expect)(out.action_detail).toContain('S02-A-T01');
        // The recoverable warn finding travels with the output.
        (0, vitest_1.expect)(out.findings.some((f) => f.code === 'RUNTIME.SCHEMA_MISMATCH' && f.severity === 'warn')).toBe(true);
    });
});
(0, vitest_1.describe)('row 10c: first unchecked task → DISPATCH_WORKER mode=implement-task (real fixture)', () => {
    (0, vitest_1.it)('partial project → implement-task for the first unchecked task', () => {
        const { fx } = partialFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('DISPATCH_WORKER');
        (0, vitest_1.expect)(out.action_detail).toContain('mode=implement-task');
        (0, vitest_1.expect)(out.action_detail).toContain('S02-A-T01');
    });
});
(0, vitest_1.describe)('row 10d: all tasks checked but slice not READY_FOR_CV → finalize-slice (real fixture)', () => {
    (0, vitest_1.it)('all checked + finalized evidence, no finalize-slice TASK_COMPLETE → mode=finalize-slice', () => {
        const { fx } = finalizeFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('DISPATCH_WORKER');
        (0, vitest_1.expect)(out.action_detail).toContain('mode=finalize-slice');
        (0, vitest_1.expect)(out.action_detail).toContain('S02-A');
    });
});
(0, vitest_1.describe)('row 10 dependency handling: skip dependency-blocked slices, jump to the first runnable one (real fixture)', () => {
    (0, vitest_1.it)('first slice depends on the second (not complete) → the second slice is dispatched', () => {
        const { fx } = depJumpFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('DISPATCH_WORKER');
        (0, vitest_1.expect)(out.action_detail).toContain('mode=implement-task');
        (0, vitest_1.expect)(out.action_detail).toContain('S02-B');
        (0, vitest_1.expect)(out.action_detail).not.toContain('S02-A-T01');
    });
});
// ============================================================
// Row 11 — all slices integrated without GATE_PASS → RUN_GATE
// ============================================================
(0, vitest_1.describe)('row 11: all slices integrated without GATE_PASS → RUN_GATE (real fixture)', () => {
    (0, vitest_1.it)('bound commit + bound integration, no gate receipt → RUN_GATE', () => {
        const { fx } = integratedFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('RUN_GATE');
        (0, vitest_1.expect)(out.responsible_role).toBe('executor');
        (0, vitest_1.expect)(out.action).not.toMatch(/DISPATCH/);
    });
});
// ============================================================
...
//           → FINALIZE_STAGE_REVIEW
// ============================================================
(0, vitest_1.describe)('row 12: UNDER_REVIEW + GATE_PASS → FINALIZE_STAGE_REVIEW (real fixture)', () => {
    (0, vitest_1.it)('counterexample: gate passed but review not finalized → FINALIZE_STAGE_REVIEW, no RUN_GATE loop', () => {
        const { fx } = gateFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('FINALIZE_STAGE_REVIEW');
        (0, vitest_1.expect)(out.responsible_role).toBe('stage-reviewer');
        (0, vitest_1.expect)(out.action).not.toBe('RUN_GATE');
        (0, vitest_1.expect)(out.action).not.toBe('PREPARE_STAGE_REVIEW');
    });
});
// ============================================================
// GATE_INTERRUPTED — additive 13th receipt type retry semantics (PO-S05-A-06)
// ============================================================
(0, vitest_1.describe)('GATE_INTERRUPTED: retryable gate, never GATE_FAIL/GATE_PASS (PO-S05-A-06)', () => {
    (0, vitest_1.it)('all slices integrated + GATE_INTERRUPTED only → RUN_GATE (retry), no VALIDATE block', () => {
        const { fx } = integratedFx();
        fx.writeReceipt('stage-gate', undefined, {
    ...
    (0, vitest_1.it)('GATE_INTERRUPTED is never derived as GATE_PASS: no FINALIZE_STAGE_REVIEW', () => {
        const { fx } = integratedFx();
        fx.writeReceipt('stage-gate', undefined, {
            type: 'GATE_INTERRUPTED',
            stage_id: fx.stageId,
    ...
    (0, vitest_1.it)('old 12-type receipts keep the derivation unchanged: GATE_PASS still → FINALIZE_STAGE_REVIEW', () => {
        const { fx } = gateFx();
        const out = pipeline(fx, fx.stageId);
        (0, vitest_1.expect)(out.action).toBe('FINALIZE_STAGE_REVIEW');
    });
...
// ============================================================
(0, vitest_1.describe)('row 13: all rows missed (all blocked) → VALIDATE fallback (real fixture)', () => {
    (0, vitest_1.it)('mutually dependency-blocked slices → VALIDATE with the blocking DOMAIN.INVALID_TRANSITION finding', () => {
...
// ============================================================
(0, vitest_1.describe)('PO-S02-D-03: restart determinism on additional fixtures (HP-003)', () => {
    (0, vitest_1.it)('CV_REPAIR (repair) fixture: two fresh service instances deep-equal', () => {
        const { fx } = repairFx();
        const input = { projectRoot: fx.root, stageId: fx.stageId };
        ...
        (0, vitest_1.expect)(first.action).toBe('DISPATCH_WORKER');
        (0, vitest_1.expect)(first.action_detail).toContain('mode=repair');
    });
    (0, vitest_1.it)('all-blocked fixture: two fresh service instances deep-equal, findings included', () => {
        const { fx } = allBlockedFx();
        const input = { projectRoot: fx.root, stageId: fx.stageId };
        const first = new runtime_1.NextActionService().nextAction(input);
        const second = new runtime_1.NextActionService().nextAction(input);
        expectContract(first);
        (0, vitest_1.expect)(second).toEqual(first);
        (0, vitest_1.expect)(first.action).toBe('VALIDATE');
        (0, vitest_1.expect)(first.findings.length).toBeGreaterThan(0);
    });
});
//# sourceMappingURL=next-action-matrix.spec.js.map
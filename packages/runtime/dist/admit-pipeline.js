...
exports.defaultReceiptWriter = void 0;
exports.runAdmitPipeline = runAdmitPipeline;
exports.admitSpvResult = admitSpvResult;
exports.admitGateResult = admitGateResult;
exports.admitGateInterrupted = admitGateInterrupted;
...
const admission_request_1 = require("./admission-request");
/** Default port — kernel ReceiptWriter (temp → fsync → rename → digest verify → chain + lock). */
exports.defaultReceiptWriter = {
    write: (data, options) => (0, kernel_1.writeReceipt)(data, options),
    verifyChain: (receiptDir) => (0, kernel_1.verifyReceiptChain)(receiptDir),
};
// ============================================================
// Unified admit pipeline
// ============================================================
/** Structured rejection builder — canonical Finding + no Receipt ref. */
function reject(code, message, newState) {
    return {
        accepted: false,
        receipt_ref: null,
        new_state: newState,
...
function chainFailureDetail(result) {
    if (result.brokenLink) {
        return (`broken link at index ${result.brokenLink.index}: ` +
 ...
 */
function stageNotFoundAttribution(stageId, findings) {
    const lines = [];
    let manifestBranch = false;
    for (const f of findings) {
        if (f.message.startsWith('manifest source unavailable for stage')) {
            manifestBranch = true;
            lines.push(`stage "${stageId}" is not declared in the manifest (manifest missing or stage_id mismatch): ${f.message}`);
            continue;
        }
        let source;
 ...
 */
function resolveCategoryChainTip(receiptDir) {
    let entries;
    try {
    ...
    for (const file of files) {
        try {
            const parsed = JSON.parse(fs.readFileSync(path.join(receiptDir, file), 'utf-8'));
 ...
 */
function runAdmitPipeline(input) {
    const writer = input.writer ?? exports.defaultReceiptWriter;
    // ── 1. Request schema validation — fail closed (AWI-006: 验证输入) ──
    let request;
    try {
        (0, admission_request_1.assertAdmissionRequest)(input.request);
        request = input.request;
    }
    catch (err) {
        if (err instanceof kernel_1.SchemaValidationError) {
            return reject('RUNTIME.SCHEMA_MISMATCH', `admission request rejected: ${err.message}`, null);
        }
        throw err;
    }
    ...
    let state;
    try {
        state = input.reconcile((0, admission_request_1.admissionRequestStageId)(request));
    }
    catch (err) {
        const reason = err instanceof Error ? err.message : String(err);
        return reject('RUNTIME.SCHEMA_MISMATCH', `reconcile failed for stage "${(0, admission_request_1.admissionRequestStageId)(request)}": ${reason}`, null);
    }
    ...
    const stageNotFound = state.findings.filter((f) => f.code === 'DOMAIN.STAGE_NOT_FOUND' && f.severity === 'error');
    if (stageNotFound.length > 0) {
        return reject('DOMAIN.STAGE_NOT_FOUND', stageNotFoundAttribution(state.stage_id, stageNotFound), state);
    }
    ...
    if (!preChain.valid) {
        return reject('RUNTIME.RECEIPT_CHAIN_BROKEN', `receipt chain broken in ${targetDir} before admit: ${chainFailureDetail(preChain)}`, pre.nextState);
    }
    ...
    try {
        ...
        const code = err instanceof kernel_1.ReceiptChainError
            ? 'RUNTIME.RECEIPT_CHAIN_BROKEN'
            : 'RUNTIME.SCHEMA_MISMATCH';
        return reject(code, `writeReceipt failed for ${targetDir}: ${err instanceof Error ? err.message : String(err)}`, pre.nextState);
    }
    // ── 7. Post-write chain verification ──
    const postChain = writer.verifyChain(targetDir);
    if (!postChain.valid) {
        return reject('RUNTIME.RECEIPT_CHAIN_BROKEN', `receipt chain broken in ${targetDir} after admit: ${chainFailureDetail(postChain)}`, pre.nextState);
    }
    // ── 8. Result ──
    return {
        accepted: true,
...
function refusePrecheck(code, message) {
...
function spvGateToStageResult(next, source) {
    return {
        ...next,
 ...
 */
function advanceStageTo(state, action, reduce, reached) {
    let next;
    try {
        ...
        if (err instanceof kernel_1.InvalidTransitionError) {
            return refusePrecheck('DOMAIN.INVALID_TRANSITION', `state advance refused for stage "${state.stage_id}" via ${action.entity}.${action.event}: ${err.message}`);
        }
        throw err;
    }
    if (!reached(next.stage_state)) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `state advance via ${action.entity}.${action.event} did not reach the expected target for stage "${state.stage_id}"`);
    }
 ...
 */
function boundManifestDigest(requestStageId, requestDigest, deps) {
 ...
 */
function admitSpvResult(request, deps) {
...
function spvResultSteps(request, deps) {
 ...
 */
function spvResultPrecheck(state, request, deps, reduce) {
    if (state.stage_state !== kernel_1.StageState.PLANNING) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `stage "${state.stage_id}" is ${state.stage_state}, expected PLANNING for SPV admit`);
    }
    const canonical = boundManifestDigest(request.stageId, request.manifestDigest, deps);
    if (canonical === null) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `manifest digest binding mismatch for stage "${state.stage_id}": request "${request.manifestDigest}" does not match the canonical digest (or the manifest is unavailable)`);
    }
    return advanceStageTo(state, { entity: 'stage', event: 'FINALIZE_PLAN' }, reduce, (s) => s === kernel_1.StageState.READY);
}
 ...
 * @param deps    - projectRoot / reconcile / reduce / writer seams.
 */
function admitGateResult(request, deps) {
    return runAdmitPipeline({
        request,
        reconcile: deps.reconcile ?? ((stageId) => (0, reconcile_1.reconcileStage)({ projectRoot: deps.projectRoot, stageId })),
        writer: deps.writer,
...
function gateResultSteps(request, deps) {
    const reduce = deps.reduce ?? reducer_1.reduceRuntimeAction;
    return {
        ...
        targetDir: () => (0, receipt_layout_1.stageGateReceiptDir)(deps.projectRoot, request.stageId),
    };
}
/**
 ...
 */
function gateResultPrecheck(state, request, deps, _reduce) {
    ...
    if (state.slices.length === 0 || notIntegrated.length > 0) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `stage gate refused: not every slice is integrated. ` +
    ...
    if (state.stage_state !== kernel_1.StageState.UNDER_REVIEW) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `stage "${state.stage_id}" is ${state.stage_state}, expected UNDER_REVIEW for gate admit`);
    }
    // 3. git-clean working tree.
    let clean;
    try {
        const porcelain = (0, node_child_process_1.execFileSync)('git', ['status', '--porcelain'], {
    ...
    if (!clean) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `stage gate refused: working tree is not clean in "${deps.projectRoot}" (git status --porcelain non-empty)`);
    }
    // 4. HEAD binding — the gate run must be bound to the current HEAD.
    let head = null;
    try {
        head = (0, node_child_process_1.execFileSync)('git', ['rev-parse', 'HEAD'], {
            cwd: deps.projectRoot,
            encoding: 'utf-8',
        }).trim();
    }
    ...
    if (head === null || request.snapshotDigest !== head) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `stage gate refused: request snapshotDigest "${request.snapshotDigest}" does not match the current git HEAD "${head ?? 'unresolvable'}"`);
    }
 ...
 * @param deps    - projectRoot / reconcile / reduce / writer seams.
 */
function admitGateInterrupted(request, deps) {
    return runAdmitPipeline({
        request,
        reconcile: deps.reconcile ?? ((stageId) => (0, reconcile_1.reconcileStage)({ projectRoot: deps.projectRoot, stageId })),
        writer: deps.writer,
...
function gateInterruptedSteps(request, deps) {
    const reduce = deps.reduce ?? reducer_1.reduceRuntimeAction;
    return {
 ...
 */
function gateInterruptedPrecheck(state, request, deps, _reduce) {
    // 1. all-integrated (a zero-slice stage carries no integration evidence).
    const notIntegrated = state.slices.filter((s) => !s.integrated);
    if (state.slices.length === 0 || notIntegrated.length > 0) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `stage gate interruption refused: not every slice is integrated. ` +
            `Non-integrated slices: ${notIntegrated.map((s) => s.slice_id).join(', ')}`);
    }
    // 2. stage must be derived UNDER_REVIEW (the gate runs between execution
    //    and review).
    if (state.stage_state !== kernel_1.StageState.UNDER_REVIEW) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `stage "${state.stage_id}" is ${state.stage_state}, expected UNDER_REVIEW for gate interruption admit`);
    }
    ...
    if (head === null || request.snapshotDigest !== head) {
        return refusePrecheck('DOMAIN.INVALID_TRANSITION', `stage gate interruption refused: request snapshotDigest "${request.snapshotDigest}" does not match the current git HEAD "${head ?? 'unresolvable'}"`);
    }
...
//# sourceMappingURL=admit-pipeline.js.map
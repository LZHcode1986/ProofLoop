...
exports.initializeSliceEvidence = initializeSliceEvidence;
exports.initializeSliceEvidenceCli = initializeSliceEvidenceCli;
 ...
 *     exclusive create.
 */
function initializeSliceEvidence(options) {
    ...
    for (const slice of manifest.slices) {
        ...
        if (!match || match[1] !== manifest.stage_id || match[2] !== slice.slice_id) {
            errors.push(`Slice "${slice.slice_id}" evidence_path "${evidencePath}" does not match ` +
        ...
        const skeleton = generateEvidenceSkeleton(slice.slice_id, manifest.stage_id, slice.goal, slice.public_seam, digest);
        try {
 ...
 */
function initializeSliceEvidenceCli(argv) {
    ...
    try {
        ...
        manifest = (0, kernel_1.validateManifest)(JSON.parse(content));
    }
    catch (err) {

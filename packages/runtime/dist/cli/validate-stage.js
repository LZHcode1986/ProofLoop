...
function scanSliceRegions(text) {
    const regions = [];
    const errors = [];
    ...
    for (const line of lines) {
        ...
        if (begin) {
            if (current !== null) {
                errors.push(error('UNCLOSED_SLICE', `Slice ${begin[1]} has BEGIN while ${current.sliceId} is still open`, current.sliceId));
                continue;
            }
        ...
        if (end) {
            if (current === null) {
                errors.push(error('ORPHANED_SLICE_END', `Slice ${end[1]} has END but no BEGIN marker`, end[1]));
                continue;
            }
            if (current.sliceId !== end[1]) {
                errors.push(error('MISMATCHED_SLICE_MARKER', `SLICE:END for ${end[1]} while ${current.sliceId} region is open`, current.sliceId));
            }
            else {
    ...
    if (current !== null) {
        errors.push(error('UNCLOSED_SLICE', `Slice ${current.sliceId} has BEGIN but no END marker`, current.sliceId));
    }
 ...
 */
function structuralChecks(text, stageId) {
    const errors = [];
    const { regions, errors: markerErrors } = scanSliceRegions(text);
    errors.push(...markerErrors);
    const seen = new Map();
    const addUnique = (id, kind, sliceId) => {
        if (seen.has(id)) {
            errors.push(error('DUPLICATE_ID', `Duplicate ${kind} ID: ${id} (first seen in ${seen.get(id)})`, sliceId));
        }
        else {
    ...
    for (const region of regions) {
        ...
        const poSection = (0, compile_manifest_1.extractSection)(region.lines, 'Proof Obligations');
        for (const poId of poSection.match(PO_ID_RE) ?? []) {
            addUnique(poId, 'Proof Obligation', region.sliceId);
        }
        const tasksSection = (0, compile_manifest_1.extractSection)(region.lines, 'Tasks');
        ...
        if (poSection.trim().length > 0 && (poSection.match(PO_ID_RE) ?? []).length > 0) {
            for (const rawBlock of ('\n' + poSection).split(/\n\s*-\s*PO-/).slice(1)) {
                ...
                for (const line of fullBlock.split('\n')) {
                    ...
                    if (fm)
                        declared.set(fm[1].trim(), fm[2].trim());
                }
                const fieldCheck = [
                ...
                for (const fc of fieldCheck) {
                    ...
                    if (found === undefined || found.length === 0) {
                        errors.push(error(fc.type, `PO ${poId} in slice ${region.sliceId} is missing ${fc.label} or has empty value`, region.sliceId));
                    }
                }
            }
        }
        // Risk facts vocabulary (fail closed on unrecognized facts).
        const riskItems = (0, compile_manifest_1.extractListItems)((0, compile_manifest_1.extractSection)(region.lines, 'Risk Facts'));
        if (riskItems.length === 0) {
            errors.push(error('MISSING_RISK_FACTS', `Slice ${region.sliceId} has no Risk Facts`, region.sliceId));
        }
        const normalized = riskItems.map(compile_manifest_1.normalizeRiskFact);
        const hasNone = normalized.includes('none');
        const hasOther = normalized.some((rf) => rf !== 'none' && rf.length > 0);
        if (hasNone && hasOther) {
            errors.push(error('RISK_FACT_NONE_WITH_OTHERS', `Slice ${region.sliceId} has 'none' combined with other Risk Facts`, region.sliceId));
        }
        for (const rf of normalized) {
            if (rf.length > 0 && !compile_manifest_1.ALL_KNOWN_RISK_FACTS.has(rf)) {
                errors.push(error('UNKNOWN_RISK_FACT', `Slice ${region.sliceId} has unrecognized Risk Fact: "${rf}"`, region.sliceId));
            }
        }
    }
    ...
    for (const taskId of text.match(TASK_ID_RE) ?? []) {
        if (!declaredTasks.has(taskId)) {
            errors.push(error('TASK_OUTSIDE_SLICE', `Task ${taskId} appears outside any slice region`));
        }
    }
    return errors;
}
...
function manifestChecks(provided, compiled, tasksText) {
    const errors = [];
    if (provided.stage_id !== compiled.stage_id) {
        errors.push(error('STAGE_ID_MISMATCH', `Manifest stage_id "${provided.stage_id}" does not match tasks.md Stage ID "${compiled.stage_id}"`));
    }
    ...
    if (missing.length > 0 || extra.length > 0) {
        errors.push(error('SLICE_SET_MISMATCH', `Manifest slice set differs from the compiled slice set: missing ${JSON.stringify(missing)}, extra ${JSON.stringify(extra)}`));
    }
    const seenSliceIds = new Set();
    const seenEvidence = new Set();
    for (const slice of provided.slices) {
        if (seenSliceIds.has(slice.slice_id)) {
            errors.push(error('DUPLICATE_MANIFEST_SLICE_ID', `Manifest contains duplicate slice_id: "${slice.slice_id}"`, slice.slice_id));
        }
        ...
        if (!match || match[1] !== provided.stage_id || match[2] !== slice.slice_id) {
            errors.push(error('EVIDENCE_PATH_INVALID', `Slice ${slice.slice_id} evidence_path "${slice.evidence_path}" does not match the canonical pattern`, slice.slice_id));
        }
        if (seenEvidence.has(slice.evidence_path)) {
            errors.push(error('DUPLICATE_MANIFEST_EVIDENCE_PATH', `Manifest contains duplicate evidence_path: "${slice.evidence_path}"`, slice.slice_id));
        }
...
// Evidence dir checks
// ============================================================
function evidenceDirChecks(evidenceDir, compiled) {
    const errors = [];
    ...
    for (const slice of compiled.slices) {
        ...
        if (!actualFiles.includes(fileName)) {
            errors.push(error('MISSING_EVIDENCE_FILE', `Slice ${slice.slice_id} is missing its evidence file: ${fileName}`, slice.slice_id));
        }
    }
    for (const fileName of actualFiles) {
        ...
        if (!expected.has(fileName)) {
            errors.push(error('ORPHANED_EVIDENCE_FILE', `Evidence file "${fileName}" has no matching slice in tasks.md`, fileName.replace(/\.md$/, '')));
        }
    }
    return errors;
}
 ...
 */
function validateStage(tasksPath, manifestPath, evidenceDir) {
    let text;
    try {
    ...
    const errors = structuralChecks(text, stageMatch?.[1] ?? 'unknown');
    // 2. Compile through the shared compile seam (kernel validateManifest).
    let compiled;
    try {
        compiled = (0, compile_manifest_1.compileManifest)(tasksPath);
    }
    catch (err) {
        errors.push(error('COMPILE_FAILED', `tasks.md does not compile into a kernel-valid manifest: ${err instanceof Error ? err.message : String(err)}`));
    ...
    const dfs = (node) => {
        if (cycleFound || seen.has(node))
            return;
        if (inStack.has(node)) {
            cycleFound = true;
            errors.push(error('CYCLE_DETECTED', `Dependency cycle detected involving slice ${node}`, node));
            return;
        }
        inStack.add(node);
        for (const dep of byId.get(node)?.dependencies ?? []) {
            if (!byId.has(dep)) {
                errors.push(error('UNDECLARED_DEPENDENCY', `Slice ${node} depends on "${dep}" which is not declared in this Stage`, node));
                continue;
            }
            dfs(dep);
        }
    ...
    if (manifestPath !== undefined) {
        let provided;
        try {
            provided = (0, kernel_1.validateManifest)(JSON.parse(fs.readFileSync(manifestPath, 'utf-8')));
        }
        catch (err) {
            errors.push(error('MANIFEST_LOAD_FAILED', `Cannot load/validate provided manifest "${manifestPath}": ${err instanceof Error ? err.message : String(err)}`));
            provided = compiled;
        }
 ...
 */
function validateStageCli(argv) {

...
function makeResult(state, chainValid, fields) {
    return {
 ...
 */
function findRunnableSlice(slices, predicate) {
    for (const slice of slices) {
        if (slice.complete)
            continue;
        if (!dependenciesSatisfied(slice, slices))
            continue;
        if (predicate(slice))
            return slice;
    }
 ...
 */
function cvRepairBranch(slice, state, chainValid) {
    if (slice.cv_status === kernel_1.CVStatus.PENDING_RECHECK) {
        return makeResult(state, chainValid, {
            action: 'RUN_CV',
            action_detail: `RUN_CV (recheck) for slice "${slice.slice_id}" — repair result admitted ` +
    ...
    if (slice.repair_attempt === 0) {
        return makeResult(state, chainValid, {
            ...
            action_detail: `DISPATCH_WORKER mode=repair for slice "${slice.slice_id}" — first CV repair ` +
                `(CV_REPAIR count 1)`,
    ...
    if (slice.repair_attempt === 1) {
        return makeResult(state, chainValid, {
            action: 'DISPATCH_WORKER',
            action_detail: `DISPATCH_WORKER mode=diagnose for slice "${slice.slice_id}" — CV still REPAIR ` +
    ...
    const blocking = blockingFinding(`UNRESOLVED_CV_FAILURE: slice "${slice.slice_id}" still REPAIR after ` +
        `${slice.repair_attempt} repair attempts — human intervention required`);
    return makeResult(state, chainValid, {
        action: 'VALIDATE',
        action_detail: `VALIDATE — unresolved CV failure for slice "${slice.slice_id}" ` +
...
function deriveNextAction(state) {
    ...
    if (hasErrorFinding || chainInvalid || gateFailed) {
        const synthesized = [];
        if (gateFailed) {
            synthesized.push(blockingFinding(`GATE_FAIL receipt exists for stage "${state.stage_id}" — stage gate failed; execution blocked`));
        }
        const findings = sortFindings([...state.findings, ...synthesized]);
        return {
            action: 'VALIDATE',
            action_detail: `VALIDATE — ${findings.length} finding(s) block execution ` +
    ...
    if (state.stage_state === kernel_1.StageState.UNINITIALIZED) {
        const blocking = blockingFinding(`Stage "${state.stage_id}" is UNINITIALIZED — no STAGE_PLAN receipt; ` +
            `stage planning must be initiated externally`);
        return makeResult(state, chainValid, {
            action: 'VALIDATE',
            action_detail: `VALIDATE — stage "${state.stage_id}" not planned (UNINITIALIZED, no STAGE_PLAN receipt)`,
            responsible_role: 'executor',
            findings: sortFindings([...state.findings, blocking]),
        });
    }
    ...
    if (state.stage_state === kernel_1.StageState.PLANNING) {
        return makeResult(state, chainValid, {
            ...
            action_detail: `ADMIT_SPV_RESULT — stage "${state.stage_id}" is PLANNING; ` +
                `PLANNING → READY only via spv_pass`,
            responsible_role: 'planner',
        });
    }
    // ── Row 3: stage COMPLETED → REPARTITION (3a) / COMPILE_ACCEPTANCE (3b) ──
    if (state.stage_state === kernel_1.StageState.COMPLETED) {
        if (state.repartition_requested === true) {
            return makeResult(state, chainValid, {
                action: 'REPARTITION',
                action_detail: `REPARTITION — stage "${state.stage_id}" COMPLETED with manifest ` +
        ...
        return makeResult(state, chainValid, {
            ...
            action_detail: `COMPILE_ACCEPTANCE — stage "${state.stage_id}" COMPLETED ` +
                `(STAGE_REVIEW_PASS present) with no repartition request`,
            responsible_role: 'executor',
        });
    }
    // ── Row 4: pending worker result envelope → ADMIT_WORKER_RESULT ──
    const workerEnvelopes = (state.pending_worker_result_envelopes ?? []).filter((e) => e.stageId === state.stage_id);
    if (workerEnvelopes.length > 0) {
        const first = workerEnvelopes[0];
        return makeResult(state, chainValid, {
            action: 'ADMIT_WORKER_RESULT',
            action_detail: `ADMIT_WORKER_RESULT — ${workerEnvelopes.length} worker result envelope(s) pending ` +
    ...
    const cvEnvelopes = (state.pending_cv_result_envelopes ?? []).filter((e) => e.stageId === state.stage_id);
    if (cvEnvelopes.length > 0) {
        const first = cvEnvelopes[0];
        return makeResult(state, chainValid, {
            action: 'ADMIT_CV_RESULT',
            action_detail: `ADMIT_CV_RESULT — ${cvEnvelopes.length} CV result envelope(s) pending for stage ` +
                `"${state.stage_id}"` +
                (first.sliceId !== undefined ? ` (first: slice "${first.sliceId}")` : ''),
            responsible_role: 'executor',
            slice_id: first.sliceId,
    ...
    const repairSlice = findRunnableSlice(state.slices, (s) => s.slice_state === kernel_1.SliceState.READY_FOR_CV &&
        (s.cv_status === kernel_1.CVStatus.REPAIR || s.cv_status === kernel_1.CVStatus.PENDING_RECHECK));
    if (repairSlice !== null)
        return cvRepairBranch(repairSlice, state, chainValid);
    // ── Row 7: READY_FOR_CV with no CV receipt → RUN_CV (initial) ──
    const initialCvSlice = findRunnableSlice(state.slices, (s) => s.slice_state === kernel_1.SliceState.READY_FOR_CV && s.cv_status === kernel_1.CVStatus.NOT_STARTED);
    if (initialCvSlice !== null) {
        return makeResult(state, chainValid, {
            action: 'RUN_CV',
            action_detail: `RUN_CV (initial) for slice "${initialCvSlice.slice_id}" — READY_FOR_CV with no ` +
                `CV receipt yet`,
            responsible_role: 'code-verifier',
    ...
    const commitSlice = findRunnableSlice(state.slices, (s) => s.slice_state === kernel_1.SliceState.CV_PASSED && !s.committed);
    if (commitSlice !== null) {
        return makeResult(state, chainValid, {
            action: 'ADMIT_SLICE_COMMIT',
            action_detail: `ADMIT_SLICE_COMMIT — slice "${commitSlice.slice_id}" CV_PASSED without a ` +
    ...
    const integrationSlice = findRunnableSlice(state.slices, (s) => s.slice_state === kernel_1.SliceState.INTEGRATING && !s.integrated);
    if (integrationSlice !== null) {
        return makeResult(state, chainValid, {
            action: 'ADMIT_INTEGRATION',
            action_detail: `ADMIT_INTEGRATION — slice "${integrationSlice.slice_id}" committed (SLICE_COMMIT) ` +
    ...
    for (const slice of state.slices) {
        if (slice.complete)
            continue;
        if (!dependenciesSatisfied(slice, state.slices))
            continue;
        // 10a — no task started and the evidence file is missing → initialize.
        if (slice.slice_state === kernel_1.SliceState.PLANNED && evidenceFileMissing(slice, state)) {
            return makeResult(state, chainValid, {
                action: 'INITIALIZE_EVIDENCE',
                action_detail: `INITIALIZE_EVIDENCE — slice "${slice.slice_id}" has no task started and its ` +
        ...
        for (const task of slice.tasks) {
            if (task.checked !== task.evidence_written) {
                return makeResult(state, chainValid, {
                    action: 'DISPATCH_WORKER',
                    action_detail: `DISPATCH_WORKER mode=recover-task for slice "${slice.slice_id}" task ` +
        ...
        if (firstUnchecked !== undefined) {
            return makeResult(state, chainValid, {
                action: 'DISPATCH_WORKER',
                action_detail: `DISPATCH_WORKER mode=implement-task for slice "${slice.slice_id}" task ` +
        ...
        if (slice.tasks.length > 0 &&
            ...
            slice.slice_state === kernel_1.SliceState.IN_PROGRESS) {
            return makeResult(state, chainValid, {
                action: 'DISPATCH_WORKER',
                action_detail: `DISPATCH_WORKER mode=finalize-slice for slice "${slice.slice_id}" — all tasks ` +
    ...
    if (state.slices.length > 0 &&
        ...
        return makeResult(state, chainValid, {
            action: 'RUN_GATE',
            action_detail: `RUN_GATE — all ${state.slices.length} slices integrated without a GATE_PASS receipt`,
            responsible_role: 'executor',
        });
    }
    // ── Row 12: GATE_PASS present without STAGE_REVIEW_PASS (UNDER_REVIEW)
    //           → FINALIZE_STAGE_REVIEW ──
    if (state.stage_state === kernel_1.StageState.UNDER_REVIEW && state.gate_pass_present === true) {
        return makeResult(state, chainValid, {
            action: 'FINALIZE_STAGE_REVIEW',
            action_detail: `FINALIZE_STAGE_REVIEW — stage "${state.stage_id}" UNDER_REVIEW with GATE_PASS ` +
    ...
    const blocking = blockingFinding(`No canonical next action derivable for stage "${state.stage_id}" — all slices ` +
        `blocked or no actionable state (DOMAIN.INVALID_TRANSITION)`);
    return makeResult(state, chainValid, {
        action: 'VALIDATE',
        action_detail: `VALIDATE — no canonical next action derivable from the persisted facts for stage ` +
            `"${state.stage_id}" (all blocked)`,
        responsible_role: 'executor',
        findings: sortFindings([...state.findings, blocking]),
    });
}
//# sourceMappingURL=derive-next-action.js.map
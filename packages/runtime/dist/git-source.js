...
exports.GitSourceError = void 0;
exports.resolveGitRoot = resolveGitRoot;
exports.readGitHead = readGitHead;
exports.defaultTasksMdPath = defaultTasksMdPath;
exports.extractSliceRegion = extractSliceRegion;
exports.parseTaskCheckboxes = parseTaskCheckboxes;
exports.hasTaskEvidenceWritten = hasTaskEvidenceWritten;
exports.isSliceEvidenceFinalized = isSliceEvidenceFinalized;
exports.parseEvidenceFacts = parseEvidenceFacts;
...
function resolveGitRoot(projectRoot) {
    ...
    let toplevel;
    try {
        toplevel = (0, node_child_process_1.execFileSync)('git', ['rev-parse', '--show-toplevel'], {
            cwd: resolved,
            encoding: 'utf-8',
        ...
        if (isExecutableMissing(err)) {
            throw new GitSourceError(`git executable is unavailable; cannot resolve a git work tree for ${resolved}`);
        }
...
function defaultTasksMdPath(projectRoot, stageId) {
 ...
 */
function extractSliceRegion(content, sliceId) {
 ...
 */
function parseTaskCheckboxes(content, taskIds, sliceId) {
    const region = sliceId !== undefined && sliceId.length > 0
 ...
 */
function hasTaskEvidenceWritten(content, taskId) {
 ...
 */
function isSliceEvidenceFinalized(content) {
    const lines = content.split('\n');
    let headingIdx = -1;
    for (let i = 0; i < lines.length; i++) {
...
function parseEvidenceFacts(content, taskIds) {
    return {
...
function gitSource(input) {
    ...
    const parsed = evidenceFilePresent && evidenceContent !== null

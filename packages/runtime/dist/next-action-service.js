...
const derive_next_action_1 = require("./derive-next-action");
/** Canonical pending-results directory under the project root (contract §4). */
const PENDING_RESULTS_REL = ['.pi', 'proofloop-runtime', 'results'];
...
class NextActionService {
    nextAction(input) {
        ...
        //    never a guess).
        const reconciled = (0, reconcile_1.reconcileStage)({
            projectRoot,
            stageId,
        ...
        const derived = (0, derive_next_action_1.deriveNextAction)({ ...reconciled, ...extras });
        ...
        const actionDetail = derived.action === 'ADMIT_WORKER_RESULT' && pendingEnvelopes.length > 0
 ...
 */
function buildExtras(reconciled, input) {
    ...
    try {
        ...
        for (const slice of manifest.slices) {
            present[slice.slice_id] = fs.existsSync(path.join(input.projectRoot, slice.evidence_path));
        }
    ...
    const gate = (0, receipt_reader_1.readReceiptCategory)({
        projectRoot: input.projectRoot,
        category: 'stage-gate',
        stageId: input.stageId,
    });
 ...
 */
function readPendingWorkerEnvelopes(projectRoot, stageId) {
    ...
    for (const name of [...names].sort()) {
        ...
        try {
            const envelope = (0, relay_contract_1.validateWorkerResultEnvelope)(data);
            if (envelope.stageId === stageId)
...
//# sourceMappingURL=next-action-service.js.map
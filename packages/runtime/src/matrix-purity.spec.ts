...
describe('module-level dependency scan (PO-S02-A-04)', () => {
   ...
   *    (compile / run E2E / finalize review), and with
    *    ./plan-services + ./vnext/candidate-input + the explicit vNext CLI
     *    seams when the active candidate plan path was added (v1 stays
     *    unchanged; v2 routing remains additive), and with ./vnext when S0-A
     *    task-2 added the versioned vNext Runtime seam
     *    (entity resolver + vNext Manifest compiler + kernel vNext validator
     *    re-exports) that Planners / Executors consume to compile the kernel
     *    vNext contract (additive; v1 surface untouched), and with
     *    ./cli/proofloop-common + ./cli/proofloop when S10-A-T01 added the
     *    public harness-neutral `proofloop` CLI seam base (closed registry,
     *    canonical envelope, exit contract, root assertion).
     */
  const EXPECTED_IMPORT_SETS: Readonly<Record<string, ReadonlySet<string>>> = {
    ...
    'index.ts': new Set(['@proofloop/kernel', './state-model', './reducer', './stage-state', './relay-contract', './worker-step-service', './receipt-layout', './receipt-reader', './git-source', './manifest-source', './reconcile', './derive-next-action', './next-action-service', './admission-request', './admit-pipeline', './admission', './process-runner', './platform-adapter', './project-acceptance', './cli/initialize-vnext-slice-evidence', './cli/validate-vnext-stage', './cli/vnext-cli-support-vnext', './cli/proofloop', './cli/proofloop-common', './plan-services', './vnext', './vnext/candidate-input']),
  };

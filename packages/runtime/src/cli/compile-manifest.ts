 ...
 */
export function extractSection(lines: readonly string[], heading: string): string {
 ...
 */
function extractSliceRegions(text: string): SliceRegion[] {
  ...
  for (const line of lines) {
    ...
    if (begin) {
      if (current !== null) {
        throw new Error(
          `Slice region "${begin[1]}" opened while "${current.sliceId}" is still open (nested or missing END marker)`,
        );
      }
    ...
    if (end) {
      ...
      if (current.sliceId !== end[1]) {
        throw new Error(
          `Mismatched SLICE:END for "${end[1]}" while "${current.sliceId}" region is open`,
        );
      }
 ...
 */
export function parseSliceDependencies(text: string): string[] {
  const deps: string[] = [];
  for (const item of extractListItems(text)) {
    for (const token of item.split(/\s+/)) {
      ...
      if (DEP_SEPARATORS.has(token)) continue;
...
function normalizePoKey(key: string): string {
  return key
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '');
 ...
 */
function parsePoFields(block: string): Map<string, string> {
...
function splitRiskFactList(value: string): string[] {
  return value
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
 ...
 */
function parseProofObligations(
  section: string,
  defaultPublicSeam: string,
): ProofObligation[] {
  const poBlocks = ('\n' + section).split(/\n\s*-\s*PO-/).slice(1);
  const result: ProofObligation[] = [];
...
const ALL_KNOWN_RISK_FACTS = new Set<string>([
  ...ENHANCED_RISK_FACTS,
  ...STANDARD_RISK_FACTS,
  'none',
]);
 ...
 */
export function computeCvMinimumLevel(
  ...
  for (const rf of normalized) {
    if (rf.length > 0 && !ALL_KNOWN_RISK_FACTS.has(rf)) {
      throw new Error(
        `Unknown risk fact: "${rf}". Allowed: ${[...ALL_KNOWN_RISK_FACTS].join(', ')}`,
      );
    }
  }
  if (normalized.some((rf) => (ENHANCED_RISK_FACTS as readonly string[]).includes(rf))) {
    return 'enhanced';
  }
  if (normalized.some((rf) => (STANDARD_RISK_FACTS as readonly string[]).includes(rf))) {
    return 'standard';
  }
  return 'lite';
}
 ...
 */
function parseKeyValue(line: string): { key: string; value: string; indent: number } | null {
 ...
 */
export function parseRuntimeProofSteps(section: string): RuntimeProofStep[] {
  ...
  const stack: Array<{ key: string; indent: number; data: Record<string, unknown> }> = [];

  // Legacy parity: the old compiler filled defaults for steps that omit
  // `cwd` ('.') and `timeout_ms` (300000) — kernel requires both fields.
  const finalizeStep = (step: Record<string, unknown>): RuntimeProofStep => {
    const finalized: Record<string, unknown> = { ...step };
    if (finalized['cwd'] === undefined) finalized['cwd'] = '.';
    if (finalized['timeout_ms'] === undefined) finalized['timeout_ms'] = 300000;
    return finalized as unknown as RuntimeProofStep;
  };

  const closeNested = (indent: number): void => {
  ...
  for (const line of cleaned.split('\n')) {
    ...
    if (itemMatch) {
      ...
      if (current !== null) steps.push(finalizeStep(current));
...
function buildSlice(region: SliceRegion, stageId: string): ManifestSlice {
  ...
  return {
    ...
    observable_outcome:
      extractFirstParagraph(extractSection(region.lines, 'Observable Outcome')) || '(not declared)',
    public_seam: publicSeam || '(not declared)',
    dependencies: parseSliceDependencies(extractSection(region.lines, 'Dependencies')),
    proof_obligations: parseProofObligations(
      extractSection(region.lines, 'Proof Obligations'),
      publicSeam,
    ),
...
export function compileManifest(tasksPath: string): Manifest {
  ...
  try {
    ...
    throw new Error(
      `Tasks file not found: ${tasksPath} (${err instanceof Error ? err.message : String(err)})`,
    );
  }
  const lines = text.split('\n');
 ...
 */
export function compileManifestCli(argv: readonly string[]): number {

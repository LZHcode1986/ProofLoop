...
// ============================================================
// Admission deps — the admit methods' seams (test-injectable)
// ============================================================

/** Reducer seam signature (defaults to the pure runtime reducer). */
export type AdmitReduceFn = (
...
function advanceSlice(
  ...
  sliceId: string,
  action: RuntimeAction,
  reduce: AdmitReduceFn,
  reached: (slice: ReconciledSliceState) => boolean,
): AdmitPrecheckResult {
  ...
  try {
    ...
    if (err instanceof InvalidTransitionError) {
      return refuse(
        ...
        `state advance refused for slice "${sliceId}" via ${action.entity}.${action.event}: ${err.message}`,
      );
    }
    throw err;
  }
  ...
  if (bound === undefined || !reached(bound)) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `state advance via ${action.entity}.${action.event} did not reach the expected target for slice "${sliceId}"`,
    );
  }
...
function workerResultSteps(
  ...
  return {
    precheck: (state) =>
      workerResultPrecheck(state, envelope, reduce, deps.projectRoot),
    buildReceipt: (state) => workerResultReceipt(state, envelope),
    targetDir: () => tasksReceiptDir(deps.projectRoot, envelope.stageId, envelope.sliceId),
  };
}
...
function workerResultPrecheck(
  ...
  const alreadyAdmitted = readReceiptCategory({
    projectRoot,
    category: 'tasks',
    ...
    (r) =>
      r.receipt.type === 'TASK_COMPLETE' &&
      r.receipt.payload?.['action_token'] === envelope.actionToken,
  );
  if (alreadyAdmitted) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `actionToken "${envelope.actionToken}" has already been admitted as a TASK_COMPLETE receipt for slice "${envelope.sliceId}" — duplicate admit refused`,
    );
  }
  ...
  if (slice === undefined) {
    return refuse(
      ...
      `slice "${envelope.sliceId}" not found in stage "${envelope.stageId}"`,
    );
  }
  ...
  if (envelope.outcome !== 'completed') {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `worker outcome "${envelope.outcome}" is not admissible — no TASK_COMPLETE receipt is written`,
    );
  }
  ...
  switch (envelope.mode) {
    case 'implement-task':
    case 'recover-task':
      if (slice.slice_state !== SliceState.IN_PROGRESS) {
        return refuse(
          'DOMAIN.INVALID_TRANSITION',
          `slice "${slice.slice_id}" is ${slice.slice_state}, expected IN_PROGRESS for mode ${envelope.mode}`,
        );
      }
    ...
    case 'finalize-slice':
      if (slice.slice_state !== SliceState.IN_PROGRESS) {
        return refuse(
          'DOMAIN.INVALID_TRANSITION',
          `slice "${slice.slice_id}" is ${slice.slice_state}, expected IN_PROGRESS for mode finalize-slice`,
        );
      }
      if (!slice.slice_evidence_finalized) {
        return refuse(
          'DOMAIN.INVALID_TRANSITION',
          `slice "${slice.slice_id}" evidence is not finalized — finalize-slice admit refused`,
        );
      }
      return advanceSlice(
        state,
    ...
    case 'diagnose': {
      if (slice.slice_state !== SliceState.READY_FOR_CV) {
        return refuse(
          'DOMAIN.INVALID_TRANSITION',
          `slice "${slice.slice_id}" is ${slice.slice_state}, expected READY_FOR_CV for mode ${envelope.mode}`,
        );
      }
      ...
      if (cvReceipt === null || cvReceipt.type !== 'CV_REPAIR') {
        return refuse(
          'DOMAIN.INVALID_TRANSITION',
          `slice "${slice.slice_id}" has no CV_REPAIR receipt binding — ${envelope.mode} admit refused`,
        );
      }
      if (slice.cv_status !== CVStatus.REPAIR) {
        return refuse(
          'DOMAIN.INVALID_TRANSITION',
          `slice "${slice.slice_id}" cv status is ${slice.cv_status}, expected REPAIR for ${envelope.mode} admit`,
        );
      }
      ...
      return advanceSlice(
        ...
        { entity: 'cv', event: 'FIX' },
        reduce,
        (s) => s.cv_status === CVStatus.PENDING_RECHECK,
      );
    }
  }
}
...
function cvResultSteps(
  ...
  return {
    ...
    targetDir: () => cvReceiptDir(deps.projectRoot, request.stageId, request.sliceId),
  };
}
...
function cvResultPrecheck(
  ...
  if (slice === undefined) {
    return refuse(
      'DOMAIN.STAGE_NOT_FOUND',
      `slice "${request.sliceId}" not found in stage "${request.stageId}"`,
    );
  }

  if (
    slice.slice_state !== SliceState.READY_FOR_CV &&
    slice.slice_state !== SliceState.CV_IN_PROGRESS
  ) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `slice "${slice.slice_id}" is ${slice.slice_state}, expected READY_FOR_CV or CV_IN_PROGRESS for CV admit`,
    );
  }

  let mid: ReconcileStageResult = state;
  if (slice.slice_state === SliceState.READY_FOR_CV) {
    ...
    const dispatched = advanceSlice(
      state,
      request.sliceId,
      { entity: 'slice', event: 'RUN_CV' },
      reduce,
      (s) => s.slice_state === SliceState.CV_IN_PROGRESS && s.cv_status === CVStatus.IN_PROGRESS,
    );
    if (!dispatched.accepted) return dispatched;
  ...
  if (request.verdict === 'PASS') {
    return advanceSlice(
      ...
      reduce,
      (s) => s.slice_state === SliceState.CV_PASSED && s.cv_status === CVStatus.PASS,
    );
  }
  return advanceSlice(
    mid,
    ...
    reduce,
    (s) => s.slice_state === SliceState.READY_FOR_CV && s.cv_status === CVStatus.REPAIR,
  );
}
 ...
 * @param deps    - projectRoot / reconcile / reduce / writer seams.
 */
export function admitSliceCommit(
...
function sliceCommitSteps(
  ...
  return {
    ...
    targetDir: () => committerReceiptDir(deps.projectRoot, request.stageId, request.sliceId),
  };
}
...
function sliceCommitPrecheck(
  ...
  if (slice.slice_state !== SliceState.CV_PASSED) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `slice "${slice.slice_id}" is ${slice.slice_state}, expected CV_PASSED for commit admit`,
    );
  }
  const cvReceipt = slice.latest_cv_receipt;
  if (
    ...
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `slice "${slice.slice_id}" cv receipt digest binding invalid for commit admit`,
    );
  }
  return advanceSlice(
    state,
    ...
    reduce,
    (s) => s.slice_state === SliceState.INTEGRATING,
  );
}
 ...
 * @param deps    - projectRoot / reconcile / reduce / writer seams.
 */
export function admitIntegration(
...
function integrationSteps(
  ...
  return {
    ...
    targetDir: () => integrationReceiptDir(deps.projectRoot, request.stageId, request.sliceId),
  };
}
...
function integrationPrecheck(
  ...
  if (slice.slice_state !== SliceState.INTEGRATING) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `slice "${slice.slice_id}" is ${slice.slice_state}, expected INTEGRATING for integration admit`,
    );
  }
  ...
  if (commitReceipt === null || commitReceipt.type !== 'SLICE_COMMIT') {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `slice "${slice.slice_id}" has no SLICE_COMMIT receipt binding — integration admit refused`,
    );
  }
  ...
  if (
    ...
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `slice "${slice.slice_id}" commit SHA mismatch — request "${request.commitSha}" does not match the SLICE_COMMIT receipt "${String(recordedSha)}"`,
    );
  }
  return advanceSlice(
    state,
    ...
    reduce,
    (s) => s.slice_state === SliceState.INTEGRATED,
  );
}
 ...
 */
function reviewRepairFinding(entity: 'stage' | 'project', id: string, target: string): Finding {
  return {
    code: 'DOMAIN.INVALID_TRANSITION',
    severity: 'warn',
    message:
      `${entity} review REPAIR: ${entity} "${id}" returned to ${target} — ` +
...
function advanceProject(
  ...
  action: RuntimeAction,
  reduce: AdmitReduceFn,
  reached: (project: ProjectState) => boolean,
): AdmitPrecheckResult {
  let next: ReconciledStageState;
  try {
    next = reduce(state, action);
    ...
    if (err instanceof InvalidTransitionError) {
      return refuse(
        'DOMAIN.INVALID_TRANSITION',
        `state advance refused for project of stage "${state.stage_id}" via ${action.entity}.${action.event}: ${err.message}`,
      );
    }
    throw err;
  }
  if (!reached(next.project_state)) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `state advance via ${action.entity}.${action.event} did not reach the expected target for project of stage "${state.stage_id}"`,
    );
  }
  return { accepted: true, nextState: toStageResult(next, state) };
}
 ...
 */
export function admitStageReview(
...
function stageReviewSteps(
  ...
  return {
    ...
    targetDir: () => reviewReceiptDir(deps.projectRoot, request.stageId),
  };
}
...
function stageReviewPrecheck(
  ...
  if (state.stage_state !== StageState.UNDER_REVIEW) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `stage "${state.stage_id}" is ${state.stage_state}, expected UNDER_REVIEW for stage review admit`,
    );
  }
  if (request.verdict === 'ACCEPTED') {
    return advanceStage(
      ...
      reduce,
      (s) => s === StageState.COMPLETED,
    );
  }
  // verdict REPAIR — legal no-Receipt branch (review repair).
  const reopened = advanceStage(
    state,
    { entity: 'stage', event: 'REOPEN' },
    reduce,
    (s) => s === StageState.EXECUTING,
  );
  if (!reopened.accepted) return reopened;
  return {
    accepted: true,
 ...
 */
export function admitProjectReview(
  request: ProjectReviewAdmissionRequest,
  deps: AdmissionDeps,
): AdmitResult {
  return runAdmitPipeline({
    request,
...
function projectReviewSteps(
  ...
  return {
    ...
    targetDir: () => projectReceiptDir(deps.projectRoot),
  };
}
 ...
 */
function projectReviewPrecheck(
  state: ReconcileStageResult,
  request: ProjectReviewAdmissionRequest,
  reduce: AdmitReduceFn,
): AdmitPrecheckResult {
  if (state.project_state !== ProjectState.UNDER_REVIEW) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `project of stage "${state.stage_id}" is ${state.project_state}, expected UNDER_REVIEW for project review admit`,
    );
  }
  ...
  if (request.verdict === 'ACCEPTED') {
    return advanceProject(
      state,
      { entity: 'project', event: 'COMPLETE' },
      reduce,
      (p) => p === ProjectState.COMPLETED,
    );
  }
  // verdict REPAIR — legal no-Receipt branch (review repair).
  const reopened = advanceProject(
    state,
    { entity: 'project', event: 'REOPEN' },
    reduce,
    (p) => p === ProjectState.IN_PROGRESS,
  );
  if (!reopened.accepted) return reopened;
  return {
    accepted: true,
 ...
 */
export function admitStagePlan(
  request: StagePlanAdmissionRequest,
  deps: AdmissionDeps,
): AdmitResult {
  return runAdmitPipeline({
    request,
...
function stagePlanPrecheck(
  ...
  if (state.stage_state !== StageState.UNINITIALIZED) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `stage "${state.stage_id}" is ${state.stage_state}, expected UNINITIALIZED for STAGE_PLAN admit (duplicate plan or already planned)`,
    );
  }
  ...
  try {
    ...
    return refuse(
      'DOMAIN.STAGE_NOT_FOUND',
      `manifest digest unavailable for stage "${request.stageId}": ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  if (request.manifestDigest !== canonical) {
    return refuse(
      'DOMAIN.INVALID_TRANSITION',
      `manifest digest binding mismatch for stage "${state.stage_id}": request "${request.manifestDigest}" does not match canonical "${canonical}"`,
    );
  }

  return advanceStage(
    state,
    { entity: 'stage', event: 'PLAN' },
    reduce,
    (s) => s === StageState.PLANNING,
  );
}

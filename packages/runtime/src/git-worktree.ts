...
* - External entry: a path OUTSIDE the trust root that Git legitimately registers against this repository (for example, a worktree managed by another host or tool). Returns `null` so the seam can tolerate it: the entry is ignored for this project's list/cleanup and never inspected, removed or rewritten (EXEC-S05-CLEANUP-WORKTREE-001).
 */

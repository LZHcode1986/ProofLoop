...
vitest_1.vi.mock('node:fs', async (importOriginal) => {
    ...
    return {
        ...
        renameSync: vitest_1.vi.fn(actual.renameSync),
...
const renameSyncMock = fs.renameSync;
const rmSyncMock = fs.rmSync;
const mkdirSyncMock = fs.mkdirSync;
const cleanups = [];
(0, vitest_1.afterEach)(() => {
    // restore call-through defaults after orchestration
    readFileSyncMock.mockImplementation(REAL.readFileSync);
    renameSyncMock.mockImplementation(REAL.renameSync);
    rmSyncMock.mockImplementation(REAL.rmSync);
    mkdirSyncMock.mockImplementation(REAL.mkdirSync);
...
function writeOwnerMetadata(lockDir, owner) {
...
function readOwnerMetadataFrom(lockDir) {
    try {
...
function readOwnerPidFrom(lockDir) {
...
function makeStaleLock(receiptDir) {
...
(0, vitest_1.describe)('F1 hardening — lock ownership checks (PO-S03-I-03)', () => {
    (0, vitest_1.it)('F1: stale recovery never deletes a lock that a live winner re-acquired (interleaving)', () => {
        ...
        let ownerReads = 0;
        readFileSyncMock.mockImplementation(((file, ...args) => {
            if (String(file) === ownerPath) {
                ...
                if (ownerReads === 2) {
                    ...
                    REAL.rmSync(lockDir, { recursive: true, force: true });
                    REAL.mkdirSync(lockDir, { recursive: false });
        ...
        (0, vitest_1.expect)(readOwnerPidFrom(lockDir)).toBe(process.pid);
        // the hardened path actually re-reads the owner before deleting
        (0, vitest_1.expect)(ownerReads).toBeGreaterThanOrEqual(2);
    });
    (0, vitest_1.it)('F1: the release path never removes a lock this process no longer owns', () => {
        const receiptDir = makeReceiptDir();
        const { lockDir } = makeStaleLock(receiptDir);
        ...
        (0, vitest_1.expect)(fs.existsSync(lockDir)).toBe(true);
        (0, vitest_1.expect)(readOwnerPidFrom(lockDir)).toBe(otherPid);
    });
    (0, vitest_1.it)('F1: a stale-recovering process never removes a lock owned by a live winner (multi-process)', async () => {
        const receiptDir = makeReceiptDir();
        ...
        const child = (0, node_child_process_1.spawn)(process.execPath, ['-e', childScript, receiptDir, startedMarker, doneMarker, payloadFile], { stdio: 'ignore' });
        ...
        while (Date.now() < deadline) {
            ...
            if (fs.existsSync(doneMarker))
                break; // child finished too fast — scenario not reproduced
            await new Promise((resolve) => setTimeout(resolve, 2));
        }
        (0, vitest_1.expect)(sawChildLock, 'expected to observe the child holding the lock mid-write (heavy-payload window)').toBe(true);
        ...
        (0, vitest_1.expect)(fs.existsSync(doneMarker)).toBe(true);
        // the winner's lock must survive the child's release (ownership check)
        (0, vitest_1.expect)(fs.existsSync(lockDir)).toBe(true);
        (0, vitest_1.expect)(readOwnerPidFrom(lockDir)).toBe(process.pid);
    });
    (0, vitest_1.it)('F1 regression: a successful write still releases its own lock', () => {
        const receiptDir = makeReceiptDir();
        const lockDir = path.join(receiptDir, LOCK_DIR_NAME);
        const result = (0, receipt_writer_1.writeReceipt)(validReceipt(), { receiptDir, tempDir: receiptDir });
        (0, vitest_1.expect)(result).toBeDefined();
...
// ============================================================
vitest_1.describe.runIf(HAS_PROC)('F1∩F2 — reused-PID winner interleaving (PO-S03-I-03 repair)', () => {
    (0, vitest_1.it)('F1∩F2: a winner re-acquiring the judged-stale lock under the same PID with a matching start-time is never deleted (recoverer aborts)', () => {
        const receiptDir = makeReceiptDir();
        const lockDir = path.join(receiptDir, LOCK_DIR_NAME);
        const ownerPath = path.join(lockDir, OWNER_FILE);
        ...
        (0, vitest_1.expect)(fs.existsSync(lockDir)).toBe(true);
        (0, vitest_1.expect)(readOwnerMetadataFrom(lockDir)).toEqual({ pid: process.pid, startTimeTicks: actualStart });
        // the re-check actually re-read the metadata after the decision
        (0, vitest_1.expect)(ownerReads).toBeGreaterThanOrEqual(2);
    });
});
//# sourceMappingURL=receipt-writer.f1-ownership.spec.js.map
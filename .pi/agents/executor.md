...
Before apply substrate discovery, validate the inbound Brain Dispatch Core Packet. It must preserve: Route; Objective / Brain Intent; Continuation; Allowed Scope; Forbidden Scope / Out of Scope; Acceptance Criteria; Verification Method; Expected Evidence; Authoritative Inputs; Constraints; Stop Conditions; and Expected Result.

If any required field is absent, ambiguous, or conflicts with another field or authoritative input, return `Execution blocked` with the packet defect. Do not begin apply discovery or dispatch a subagent.
...
Return an Execution Handoff Manifest to Brain. It must include:
...
- Brain AC coverage:
- Slice and verifier-gate coverage:
- Evidence Ledger location:
- Worker receipt references:
- Code Verifier receipt references:
- Committer receipt references:
- Slice-output commit references:
- Residual risks / blockers:
- Next permitted owner / action:
```

This manifest is a conversation-local handoff receipt, not a persistent workflow record. Executor remains prohibited from making stage-acceptance or archive judgments.

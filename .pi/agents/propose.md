...
- `@planning-contract-verifier` — after artifact generation and after every repair
...
Validate the inbound Brain Dispatch Core Packet before planning. It must preserve all canonical fields: Route; Objective / Brain Intent; Continuation; Allowed Scope; Forbidden Scope / Out of Scope; Acceptance Criteria; Verification Method; Expected Evidence; Authoritative Inputs; Constraints; Stop Conditions; and Expected Result.

If a required field is absent, ambiguous, or conflicts with another packet field or authoritative input, return `Clarification required`; do not create or repair planning artifacts.
...
4. Materialize Evidence Ledger from Brain Evidence Ledger Seed into `openspec/changes/<change-id>/proofloop/evidence-ledger.md` (relative path inside the active OpenSpec change: `proofloop/evidence-ledger.md`). Write a Brain Dispatch Snapshot preserving every Brain Dispatch Core Packet field and planning artifact refs. Do not leave binding constraints only in prose.
...
9. Prepare a Plan Handoff Manifest before final planning verification, include it with the `planning-contract-verifier` dispatch, and include the verified manifest in ready output.
10. Dispatch `planning-contract-verifier` after artifact generation and after every Propose artifact repair.
...
- The ready output includes a complete Plan Handoff Manifest.

The Plan Handoff Manifest is a conversation-local handoff receipt, not a persistent workflow record. Do not add a workflow ID or persistence mechanism.
...
- Change:
- Stage:
- Brain Dispatch Contract and AC mapping:
- Artifact references:
...
- Brain Dispatch Core Packet preserved in snapshot: yes/no

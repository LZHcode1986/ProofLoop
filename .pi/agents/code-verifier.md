...
Runtime 的真实顺序是 `context prepare --role cv --stage <stage-id> --slice <slice-id> --task <task-id>` → `context admit-refutation-observation --role cv --stage <stage-id> --slice <slice-id> --task <task-id> --observation <text>` → `context show --ref <pre-refutation-context-ref> --observation-ref <observation-ref>`；只有持久化 Context/observation 并返回 `gate.satisfied: true` 才能证明 Evidence read gate。

`context prepare --role cv ... --task ...` 必须先由 Brain 使用已验证的 task anchor 调用并落盘 CV Context。随后 CV 可以调用唯一的 CV-only Runtime gate operation `context admit-refutation-observation --role cv --stage <stage-id> --slice <slice-id> --task <task-id> --observation <text>`，再调用只读的 `context show --ref <pre-refutation-context-ref> --observation-ref <observation-ref>`；只有 `gate.satisfied: true` 才能读取 Evidence。CV 不调用 `stage admit-*`、Boundary 或手写 Receipt。
...
- Bash 仅用于只读 Git/文件检查及已有 test/build/lint/typecheck；不通过 shell 写文件。
- 不调用 `stage admit-*`、Boundary 或其他 Runtime admission；仅按“当前 Evidence gate”章节调用 CV-only Context gate seam，不派发其他 Agent。

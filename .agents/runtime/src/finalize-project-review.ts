 ...
 */
function computeManifestDigest(filePath: string): string {
...
interface FinalizeInput {
  ...
  stageReceipts: StageReceiptInput[];
...
if (!fs.existsSync(input.e2eReceiptPath)) { errors.push(`E2E Receipt not found: ${input.e2eReceiptPath}`); }
else {
  ...
  catch (err) { errors.push(`Invalid E2E Receipt: ${err instanceof Error ? err.message : String(err)}`); exitError(errors); }
  ...
  if (e2e.manifest_digest !== manifestFileDigest) errors.push(`E2E manifest_digest "${e2e.manifest_digest}" != actual "${manifestFileDigest}"`);
  ...
  if (e2e.steps.length === 0) errors.push('E2E has zero executed steps');
}
...
for (const sr of input.stageReceipts) {
  ...
  if (inputStageIds.has(sid)) { errors.push(`Duplicate stage_id in input: "${sid}"`); continue; }
  inputStageIds.add(sid);

  if (!fs.existsSync(sr.review_receipt)) { errors.push(`Review receipt not found for stage ${sid}: ${sr.review_receipt}`); continue; }
  if (!fs.existsSync(sr.gate_receipt)) { errors.push(`Gate receipt not found for stage ${sid}: ${sr.gate_receipt}`); continue; }

  // Validate Review Receipt with Schema
  let review: ReturnType<typeof StageReviewReceiptSchema.parse>;
  try { review = StageReviewReceiptSchema.parse(readJson(sr.review_receipt)); }
  ...
  try { gate = StageGateReceipt.parse(readJson(sr.gate_receipt)); }
  catch (err) { errors.push(`Invalid Stage Gate for ${sid}: ${err instanceof Error ? err.message : String(err)}`); continue; }

  // Cross-validation
  if (review.stage_id !== sid) errors.push(`Review stage_id "${review.stage_id}" != input "${sid}"`);
  if (gate.stage_id !== sid) errors.push(`Gate stage_id "${gate.stage_id}" != input "${sid}"`);
...
for (const ms of manifestStageIds) {
  if (!inputStageIds.has(ms)) errors.push(`Stage "${ms}" is in Manifest but missing from finalizer input`);
}
for (const is of inputStageIds) {
  if (!manifestStageIds.has(is)) errors.push(`Stage "${is}" is in finalizer input but not in Manifest`);
}
...
for (const mc of manifestCriteria) { if (!resultCriteria.has(mc)) errors.push(`Missing criteria result for: "${mc}"`); }
for (const rc of resultCriteria) { if (!manifestCriteria.has(rc)) errors.push(`Extra criteria result not in Manifest: "${rc}"`); }
if (!input.criteriaResults.every(c => c.passed)) errors.push('Not all criteria results are passed');

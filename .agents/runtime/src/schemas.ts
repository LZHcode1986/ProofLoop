...
export type CvLifecycleState = z.infer<typeof CvLifecycleState>;
...
export const CvReceipt = z.object({
  ...
  if (data.verdict === 'REPAIR') {
    ...
    const hasLocator =
      (data.failed_po_ids && data.failed_po_ids.length > 0) ||
 ...
 */
export function isValidStageGatePassReceipt(
  receipt: unknown,
  stageId: string,
  manifestDigest?: string,
  expectedSliceIds?: readonly string[],
): receipt is StageGateReceipt {
  const parsed = StageGateReceipt.safeParse(receipt);
  if (!parsed.success || parsed.data.verdict !== 'PASS' || parsed.data.stage_id !== stageId) return false;
  if (manifestDigest && parsed.data.manifest_digest !== manifestDigest) return false;
  ...
  if (new Set(parsed.data.completed_slice_ids).size !== parsed.data.completed_slice_ids.length) return false;
  ...
  if (facts.length !== parsed.data.completed_slice_ids.length ||
      ...
      facts.some(f => !parsed.data.completed_slice_ids.includes(f.slice_id))) return false;
  if (facts.some(f => f.cv.verdict !== 'PASS' || !f.commit.commit_sha || !f.integration.integration_ref)) return false;
  const executedSteps = parsed.data.steps.filter(step => !step.skipped);
  if (executedSteps.length === 0 || executedSteps.some(step => step.exit_code !== 0)) return false;
  if (parsed.data.service_cleanup.failed.length > 0 || parsed.data.service_cleanup.remainingPids.length > 0) return false;
  ...
  if (expectedSliceIds) {
    ...
    if (actual.size !== expected.size || expected.size !== expectedSliceIds.length ||
        expectedSliceIds.some(id => !actual.has(id))) return false;
  }
  return true;
}

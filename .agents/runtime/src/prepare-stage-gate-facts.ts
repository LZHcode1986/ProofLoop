...
import type { SliceCompleteFacts } from './schemas.js';
 ...
 */
export function prepareStageGateFacts(input: ReconcileStageStateInput): string {
  ...
  if (incompleteSlices.length > 0) {
    throw new Error(
      `Cannot prepare Stage Gate facts: not every slice is complete. ` +
  ...
  if (slicesWithoutFacts.length > 0) {
    throw new Error(
      `Cannot prepare Stage Gate facts: complete slices missing slice_complete_facts. ` +
      `Affected slices: ${slicesWithoutFacts.map(s => s.slice_id).join(', ')}`
    );
  }

  // Collect facts from all slices
  const facts: SliceCompleteFacts[] = state.slices.map(s => s.slice_complete_facts!);
...
if (scriptPath && (scriptPath.endsWith('prepare-stage-gate-facts.js') || scriptPath.endsWith('prepare-stage-gate-facts.ts'))) {
  ...
  try {
    ...
    const input: ReconcileStageStateInput = JSON.parse(raw);

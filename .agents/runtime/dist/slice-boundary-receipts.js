...
import { SliceCommitReceipt, SliceIntegrationReceipt, CvReceipt, } from './schemas.js';
 ...
 */
function isGitAncestor(projectRoot, ancestor, descendant) {
    try {
 ...
 */
function validatePathBelowRoot(filePath, projectRoot) {
 ...
 */
export function findLatestCvReceipt(projectRoot, stageId, sliceId) {
...
export function findLatestSliceCommitReceipt(params) {
    ...
    for (const f of matching) {
        ...
        if (!actualDigest || actualDigest !== parsed.cv_receipt_digest)
            continue;
        // 10. Validate verified_snapshot matches the CV receipt's snapshot
        const cvData = readJsonFile(cvReceiptPath);
        if (!cvData)
            continue;
        try {
            const cvParsed = CvReceipt.parse(cvData);
            if (cvParsed.snapshot !== parsed.verified_snapshot)
                continue;
        }
        catch {
            continue;
 ...
 * @returns The validated receipt and its canonical path, or null.
 */
export function findLatestIntegrationReceipt(projectRoot, stageId, sliceId, expectedSliceCommitSha) {
    // Must have a specific slice commit SHA to match against
    if (!expectedSliceCommitSha || expectedSliceCommitSha.length === 0)
        return null;
    ...
    for (const f of matching) {
        ...
        if (parsed.stage_id !== stageId || parsed.slice_id !== sliceId)
            continue;
        // 5. Validate status
        if (parsed.status !== 'integrated')
            continue;
        // 6. Validate slice_commit_sha matches expected
        if (parsed.slice_commit_sha !== expectedSliceCommitSha)
            continue;
        // 7. Validate integrated_commit_sha === stage_head_after (schema superRefine ensures this)
        //    But double-check for safety
        if (parsed.integrated_commit_sha !== parsed.stage_head_after)
            continue;
        ...
        try {
            ...
            if (cvParsed.snapshot !== parsed.verified_snapshot)
                continue;
            if (cvParsed.slice_id !== sliceId || cvParsed.stage_id !== stageId)
                continue;
        }
        catch {
            continue;
...
//# sourceMappingURL=slice-boundary-receipts.js.map
...
const LEGACY_ROUTE_STATUSES = new Set(['REPLAN', 'BLOCKED', 'ESCALATION_REQUIRED']);
// ── Derivation engine ──────────────────────────────────────────────────────────
/** Keep the historical action_type while exposing the plan-shaped action. */
function action(actionType, fields) {
...
export function deriveNextAction(input) {
        ...
        try {
            ...
            while (true) {
                ...
                if (stat.isSymbolicLink())
                    throw new Error('symlinked receipt path');
            ...
            if (!stat.isFile())
                throw new Error('not a regular receipt file');
    ...
    const receiptIsValid = Boolean(manifestAndStageAgree && manifestFactsAvailable && stageId && stage_gate.receipt_path && persistedReceipt &&
        isValidStageGatePassReceipt(persistedReceipt, stageId, input.manifest_digest, manifestSliceIds));
    ...
    for (const slice of slices) {
        ...
        if (lifecycleStatus === 'CV_PASS') {
            ...
            if (!slice.committed)
                return action('committer', { slice_id: slice.slice_id, mode: 'slice-output', contract_ref: 'committer', contract: '.agents/contracts/executor/committer.md',
                    reason: `Slice "${slice.slice_id}" CV PASS and persisted scope outcome PASS. Committer closes the slice boundary.` });
            if (!slice.integrated)
                return action('integration', { slice_id: slice.slice_id, mode: 'post-merge', contract_ref: 'integration', contract: '.agents/contracts/brain/execute-stage.md',
 ...
 */
export function deriveInitialCvStatus(cvMinimumLevel) {
    return 'NOT_RUN';
}
/**
 * Map a CV Verdict to a CV status for the `## Current CV Status` section.
 */
export function cvVerdictToLifecycleState(verdict) {
    switch (verdict) {
        case 'PASS': return 'CV_PASS';
        case 'REPAIR': return 'CV_REPAIR_REQUIRED';
...
const VALID_CV_STATUSES = new Set([
    'NOT_RUN',
    'READY_FOR_CV',
    'CV_VERIFYING',
    'CV_REPAIR_REQUIRED',
 ...
 */
function validateDeriveInput(data) {

 ...
 */
function resolveBelowTrustedRoot(targetPath, trustRoot, expectedType) {
    try {
 ...
 */
export function resolveCanonicalArtifact(reference, trustRoot, searchPaths) {
 ...
 * @returns The real (symlink-resolved) path on success, or null on failure.
 */
export function assertRegularFileBelowTrustedRoot(targetPath, trustRoot) {
 ...
 * @returns The real (symlink-resolved) path on success, or null on failure.
 */
export function assertDirectoryBelowTrustedRoot(targetPath, trustRoot) {
 ...
 */
export function checkNoSymlinkBelowTrustRoot(targetPath, trustRoot) {
    const resolvedTarget = path.resolve(targetPath);
    const resolvedRoot = path.resolve(trustRoot);
    ...
    while (current.length > resolvedRoot.length) {
        ...
        if (parent === current || parent.length <= resolvedRoot.length)
            break;
        current = parent;
    }
    return null; // clean
}
//# sourceMappingURL=canonical-artifact-path.js.map
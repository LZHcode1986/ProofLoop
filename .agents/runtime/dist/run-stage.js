...
function hasPersistedCommit(commitSha, projectRoot) {
...
function hasCommitAncestor(commitSha, projectRoot) {
    if (!/^[a-f0-9]{40}$/i.test(commitSha))
 ...
 */
export async function executeRuntimeProof(steps, options) {
    ...
    const serviceStopRefs = new Set(steps
    ...
    if (knownPorts.length > 0) {
        ...
        if (portsStillInUse.length > 0) {
            errors.push(`Ports still in use after cleanup: ${portsStillInUse.join(', ')}. ` +
                `Cleanup failure counts as Gate FAIL.`);
            if (finalExitCode === 0)
                finalExitCode = -1;
        }
...
export async function runProjectAcceptance(manifest, outputDir, projectRoot) {
    ...
    // ── 3. Determine verdict ──
    const verdict = execResult.success ? 'PASS' : 'FAIL';
    // ── 4. Build receipt ──

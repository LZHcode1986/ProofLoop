...
const isCanonicalRoutedStatus = (status) => status.startsWith('CV_') && status !== 'CV_VERIFYING';
 ...
 */
function findCvStatusSection(content) {
 ...
 */
export function updateCurrentCvStatus(options) {
    ...
    if (!VALID_CV_LIFECYCLE_STATUSES.includes(status)) {
        return {
            success: false,
    ...
    if (latestReceiptPath) {
        ...
        if (!match)
            return { success: false, error: 'CV receipt must use immutable initial-NNN.json or recheck-NNN.json naming.', modified: false };
        ...
        try {
            ...
            if (receipt.stage_id !== stageId)
                return { success: false, error: `Receipt stage_id "${receipt.stage_id}" does not match expected stage_id "${stageId}".`, modified: false };
            if (receipt.slice_id !== sliceId)
                return { success: false, error: `Receipt slice_id "${receipt.slice_id}" does not match expected slice_id "${sliceId}".`, modified: false };
            if (receipt.verification_type !== match[1])
                return { success: false, error: `Receipt verification_type does not match filename prefix.`, modified: false };
            if (cvLevel && receipt.cv_level !== cvLevel)
                return { success: false, error: `Receipt cv_level "${receipt.cv_level}" does not match requested cvLevel "${cvLevel}".`, modified: false };
            ...
            if ((status === 'PASS' || status === 'CV_PASS') && receipt.scope_violations.length > 0)
                return { success: false, error: 'PASS receipt contains scope violations.', modified: false };
        }
        catch (err) {
    ...
    if (isCanonicalRoutedStatus(status)) {
        if (!cvLevel)
            return { success: false, error: `${status} requires cvLevel.`, modified: false };
        if (!latestReceiptPath)
            return { success: false, error: `${status} requires latestReceiptPath.`, modified: false };
    }
    // ── Read file ──
    let content;
    try {
 ...
 */
export function updateCvStatusPass(evidencePath, stageId, sliceId, deliveryRoot, cvLevel, receiptPath) {
 ...
 */
export function updateCvStatusRepair(evidencePath, stageId, sliceId, deliveryRoot, cvLevel, receiptPath, finding) {
    return updateCurrentCvStatus({
        evidencePath,
        status: 'REPAIR',
        stageId,
 ...
 */
export function updateCvStatusPendingRecheck(evidencePath, stageId, sliceId, deliveryRoot, cvLevel, receiptPath) {
    return updateCurrentCvStatus({
        evidencePath,
 ...
 */
export function updateCvStatusReadyForCv(evidencePath, stageId, sliceId, deliveryRoot) {
    return updateCurrentCvStatus({
        evidencePath,
        status: 'READY_FOR_CV',
        stageId,
 ...
 */
function validateOptionsInput(data) {
...
//# sourceMappingURL=update-current-cv-status.js.map
 ...
 */
export function resolveExecutorNextAction(input) {
...
if (isScriptEntry()) {
    ...
    try {
        ...
        // Minimal validation
        if (typeof parsed.stage_id !== 'string' ||
            typeof parsed.project_root !== 'string' ||
            typeof parsed.manifest_path !== 'string' ||
            typeof parsed.tasks_path !== 'string') {
            throw new Error('Input must contain stage_id (string), project_root (string), ' +
    ...
    try {
        ...
        console.error(`Error resolving executor next action: ${err instanceof Error ? err.message : String(err)}`);
        console.error(err);
        process.exit(1);
    }
}
//# sourceMappingURL=executor-next-action.js.map
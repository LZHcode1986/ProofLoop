...
```text
CV basis：`NORMAL` = live Slice worktree basis；`PRE_MES_BOOTSTRAP` = candidate/accepted Git Plan + baseline/current Git basis + Worker Link evidence；`MES_MAINTENANCE` = recovery candidate + live maintenance Git basis + frozen/forensic/audit binding

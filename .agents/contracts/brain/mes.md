...
写入只接受：Brain 授权的 semantic event、结构化 Result/Finding、Review verdict、Git 事实（HEAD/branch/worktree/diff）；所有正常 durable materialization 通过唯一 MES operational transaction layer。Agent narrative、Link `sent`/`idle`/`done`、pane/session 状态、checkbox 或 progress 文本不写入 MES。
...
- User presence, whether the user sent another message, and ephemeral Agent/pane/session transport state are not continuation prerequisites. Only a Brain-confirmed real `USER_DECISION_REQUIRED` creates this pause.
## status 一级视图
...
bootstrap 上下文（`PRE_MES_BOOTSTRAP`）可以携带结构化 Planner/Worker Link Results，但它们不是 MES

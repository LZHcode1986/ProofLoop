...
七个 Role Skill 与主 lifecycle 的固定映射：

| Role Skill | lifecycle | 典型行为 |
|---|---|---|
| `general` | `one-shot` | 一个 bounded direct request 返回一次 Result |
| `researcher` | `one-shot` | 一个外部技术研究请求返回一次带 sources 的 Result |
| `worker` | `continuation` | 同一 Slice lane 内执行被投影的 current Task，必要时 bounded repair；`MES_MAINTENANCE` 下只交付 Git/Link evidence，不写 MES |
...
或 Link `sent` 不能单独关闭 Planning lifecycle。

## 4. Result binding 与接纳

Result 只能由当前 Link reply 加对应 role template 进入 lifecycle。Brain 在接纳或
...
- Agent 不生成、不裁剪、不从 Agent Name、pane/session、Link message id 或其他
...
- Reviewer、pane/session/Link trust 丢失，target/basis/Goal/Authority/Plan 重大变化，
...
与 Git basis 重建，直接进入 recovery/fresh，不依赖 hidden session。不恢复旧 Agent Name、
pane id、transcript、claim clarification、next-action 或 credential 链。

## 8. Recall 与 reset

在任何 `herdr_link_close` 或 Agent recall 决策前，Brain 必须按以下顺序执行：
...
4. 只有 close 条件成立且没有 pending action/reply 时才调用 `herdr_link_close`；否则 retain/continue。

§3 matrix 是 retain/continuation/close/reset 的唯一 normative owner；本节不重复 role-specific close predicate。

单个 Task 完成、`idle`、`done`、暂时没有下一个动作或 Link `sent` 都不能触发 recall。关闭使用稳定 Agent Name；Link 只传消息，不负责业务完成判断。
...
启动新 Agent（placement 由 Link 机械创建，Brain 不持有 raw pane/tab/workspace ID）；旧 Agent Name、
pane/session、claim clarification 和旧 Result 不作为新 cycle 的 authority。

## 9. Lifecycle completion

每个 lifecycle decision 完成前，必须证明：

- 当前 branch 的最小 binding、Role Skill、Plan/Authority（SPV 包含 referenced Map entry）、Git basis 和
  scope 已由对应 owner 核对；

...
Worker 的 repair packet/result schema 由 `.agents/skills/proofloop-execute/references/worker-template.md` 唯一拥有，Worker 的 repair 步骤由 `.agents/skills/worker/SKILL.md` 唯一拥有。本 adapter 不复制这些 schema 或步骤，不定义新的 Runtime action、MES fact kind、状态、route、Gate、Receipt、Context schema 或独立 result store。
...
- Worker repair procedure **唯一**读取 `.agents/skills/worker/SKILL.md#repair 顺序`；本 adapter 不重复 RED、复现、最小修复、测试或 scope 操作。

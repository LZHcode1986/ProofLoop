...
- **Manifest path** — path to the compiled Stage Manifest (`.proofloop/manifests/<stage-id>.json`). The Manifest declares per-Slice `evidence_path` entries and `cv_minimum_level`. Slice COMPLETE facts (CV PASS receipt refs, commit SHAs with `receipt_ref`, and integration refs) are supplied to `run-stage.js` as a separate JSON array, not embedded in the Manifest itself. Facts are generated by the Runtime via `prepare-stage-gate-facts.js`, not manually constructed by the Executor.
...
Facts are generated by the Runtime, not by the Executor:

1. `node .agents/runtime/dist/prepare-stage-gate-facts.js <reconcile-input.json>`
2. `node .agents/runtime/dist/run-stage.js <manifest> <facts> <output-dir> <project-root>`

The `SliceCompleteFacts` schema includes `commit.receipt_ref` (a path to the
persisted Committer Receipt) alongside `commit.commit_sha`, `cv.verdict`,
`cv.receipt_ref`, and `integration.integration_ref`.

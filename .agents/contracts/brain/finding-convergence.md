...
通过 `.agents/contracts/brain/workflow.md` 的条件 pointer 在以下分支加载本 Contract：
...
`route_code`、`claimed_route_code`、Finding ID、出现轮次和 Agent Name 都不是 family identity。
...
以下是 Brain 的派生分析工作表，用于 dispatch 与复查判断；它不是 Link envelope、MES fact、Plan 字段或新的 durable object：
...
此表只规定 convergence core 选择 adapter 的信息；各 Role Skill、lifecycle Contract 和 packet Template 继续拥有具体步骤、binding 与 schema。

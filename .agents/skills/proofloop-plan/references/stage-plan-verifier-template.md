...
SPV 是 review-loop Role，Brain 以 `role_skill/config_agent = stage-plan-verifier` 经 Herdr Link configured start 启动；
`proofloop-plan` 仅表示 Planning caller。独立审查 procedure（验证顺序、challenge 方法、结果纪律）唯一来源是
`.agents/skills/stage-plan-verifier/SKILL.md`；本模板统一以 Skill pointer 引用，不形成第二方法源，
...
- Result binding 按 mode 显式：`NORMAL` 是 pre-accept `PLANNING_VERIFICATION_RESULT` Link reply，由 Brain 接纳/授权并交 MES transaction layer materialize；`PRE_MES_BOOTSTRAP` 是结构化 Link evidence（binding = execution_mode + authority_refs + candidate Git Plan ref + git_basis + actionToken），不写 MES、不指向 MES resultRef；`RECOVERY_REBASELINE` 同样是结构化 Link evidence，`PLAN_READY` 在 maintenance/recovery closure 前不写 MES。三种 mode 下 SPV 都全程 read-only。
...
SPV 独立初审顺序与 challenge 方法的唯一来源是 `.agents/skills/stage-plan-verifier/SKILL.md`；

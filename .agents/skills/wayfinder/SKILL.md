...
**Where the map, its child tickets, blocking, and frontier queries physically live is tracker-specific.** This repo uses the local-markdown tracker: consult `references/issue-tracker-local.md` for how the map, tickets, blocking, and the frontier are expressed here.
...
3. Resolve it — **zoom as needed**: fetch the full body of any related or closed ticket on demand; invoke the skills the `## Notes` block names. If in doubt, run the interview per `prd-to-ai-architecture/references/grilling-protocol.md`.

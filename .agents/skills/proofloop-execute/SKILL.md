...
2. Host 的 `.pi/agents/worker.md` 或 `.opencode/agents/worker.md`：Worker 执行被投影 current Task 的行为与 `SLICE_CANDIDATE_READY` 完成标准；Worker 在 bounded execution scope 内决定 HOW。
...
3. **WORKER**：Worker 按 recovery candidate Plan 的 Task graph 执行 Brain running `proofloop-execute` 投影的 current Task；每个 Result/ACK 是 Git/Subagent transport evidence，Brain 接纳但不写 MES；Worker loss 从 Git/Plan/forensic/audit/frozen tuple fresh recovery。
...
3. **START Slice lane**：Execute 负责将 accepted Thin Plan + MES/Git/current dependency outputs（`PRE_MES_BOOTSTRAP` 为 Git-tracked Plan + bound normative refs/Git facts）投影为 Slice Work Packet（包含 Slice scope、`slice_task_ids`、依赖起点等输入；不新增 Map packet 字段），交给所选 Host 的 `worker` Agent 启动 lane。Brain 提供 binding/refs/mutation boundary，不提供重写后的 semantic implementation instructions；Worker 拥有当前 Task 的 HOW。Brain 启动 lane；lane 启动后每个 Step 的 current Task 选择与投影由 Brain running `proofloop-execute` 执行。完成标准：Worker 接管该 Slice 内部 Task loop 并返回绑定 lane 的首个结构化 Result/evidence；`NORMAL` Result 由 MES operational transaction layer materialize，bootstrap 不写 MES。
...
- Preflight 与 Task loop 使用 Git + Git-tracked Plan + Technical Authority bindings：Worker 按 bootstrap Task packet
  （见 worker-template 的 `execution_mode` 分支）实现，Task Result 是结构化 Subagent transport evidence，不读/写/声称 MES

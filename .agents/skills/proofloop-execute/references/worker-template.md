...
envelope 的字段约束。Worker 的执行顺序与完成标准以 `.agents/skills/worker/SKILL.md` 为唯一事实源。
`herdr-link` 是唯一 Agent-to-Agent communication invariant；runtime launch configuration belongs to Herdr Link `.agents/agent_config.json`; this Skill does not define it. 本模型无 Manifest/Context/Receipt/
admission；`NORMAL` Task Result 是交由 MES operational transaction layer materialize 的 MES execution record，`PRE_MES_BOOTSTRAP` 与 `MES_MAINTENANCE` Task Result 都是 Git-bound structured Link evidence，不是旧 Runtime credential；`MES_MAINTENANCE` 不写 MES、不指向 MES `resultRef`。
...
`herdr-link` transport 必须产出同一闭集 envelope；camelCase 字段如下：
...
resultRef: <NORMAL=root-relative-MES-result-ref; PRE_MES_BOOTSTRAP/MES_MAINTENANCE=omitted — 结构化 Link evidence，不指向 MES Result>
changedFiles: []
verificationRuns: []
...
```text
per-Task         → completed + taskId + Task Result（`NORMAL` 作为 semantic event 经 MES transaction layer materialize；`PRE_MES_BOOTSTRAP` / `MES_MAINTENANCE` 为结构化 Link evidence，不写 MES）或结构化 blocker
...
`PRE_MES_BOOTSTRAP` / `MES_MAINTENANCE` 下没有 MES Result：Task Result 是携带同源字段的结构化 Link evidence（stage/slice/task 如适用、candidate/accepted 或 recovery candidate Git Plan ref、authorityRefs、git basis、maintenanceBinding（MES_MAINTENANCE）与 actionToken），不写 MES、不指向 MES resultRef。
